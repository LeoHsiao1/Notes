...
- v1.24
  - 于 2022 年 5 月发布。
  - beta 阶段的 API 不再默认启用。
  - [delete] 移除 dockershim 模块，不能继续使用。
...
- v1.33
  ...
  - [GA] kube-proxy 新增 nftables 代理模式。
...
- v1.35
  - 于 2025 年 12 月发布。
  - [GA] 支持动态修改 Pod 的 resources 资源配额，不必重建 Pod 。
  - [GA] kubelet 增加配置参数 maxParallelImagePulls ，控制最多并行拉取多少个镜像。
  - [GA] kubelet 增加配置参数 imageMaximumGCAge ，用于删除超过一段时间未使用的镜像。
  - [delete] 移除 Cgroup v1 模块，只能使用 Cgroup v2 。
  - [deprecated] 弃用 Nginx Ingress ，因为维护者不足。
  - [deprecated] kube-proxy 弃用 ipvs 代理模式，推荐使用 nftables 代理模式。
- v1.36
  - 于 2026 年 4 月发布。
  - [GA] 允许将 OCI image 作为 volume 挂载到 Pod 中。配置示例：
    ```yml
    kind: Pod
    metadata:
      name: curl
    spec:
      ...
      - name: curl
        ...
        volumeMounts:
        - name: vol-nginx
          mountPath: /path  # 将 OCI image 的全部文件，挂载到该目录下
      volumes:
      - name: vol-nginx
        image:
...
- 在 Rancher 的 Cluster Management 页面，可以添加 k8s 集群让 Rancher 托管。
  - 从托管 k8s 中移除一个节点时，需要清空节点上的数据，否则重新添加该节点时可能出错。参考 [官方文档](https://ranchermanager.docs.rancher.com/how-to-guides/new-user-guides/manage-clusters/clean-cluster-nodes) 。
    ...
    services:                                   # 配置各个 k8s 组件
      ...
      kube-api:
        ...
        # default-not-ready-toleration-seconds: 300
          # 创建每个 Pod 时，默认添加对 node.kubernetes.io/not-ready 的容忍度，并设置 tolerationSeconds 时长
        # default-unreachable-toleration-seconds: 300
          # 创建每个 Pod 时，默认添加对 node.kubernetes.io/unreachable 的容忍度，并设置 tolerationSeconds 时长
      kube-controller:
        ...
        # node-monitor-period: 5s               # node controller 每隔多久从 apiserver 获取一次各节点的状态
        # node-monitor-grace-period: 40s        # 如果不能获取某个节点的状态，持续一定时长，则将该节点的 status.conditions.Ready 赋值为 Unknown
        # pod-eviction-timeout: 5m
          # 如果某个节点的 Ready 状态为 False 或 Unknown ，持续一定时长，则驱逐该节点上的 Pod
          # k8s v1.25 弃用了该配置参数，不再由 kube-controller 直接驱逐 Pod ，而是给节点添加 node.kubernetes.io/unreachable 污点，从而驱逐 Pod
          # apiserver 创建每个 Pod 时，默认添加了对 node.kubernetes.io/unreachable 的容忍度，但 tolerationSeconds 为 300 秒
      kubelet:
