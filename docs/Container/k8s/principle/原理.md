...
- apiserver 处理客户端发来的 HTTP 请求时，可以在以下多个阶段记录审计日志（audit log）。
  ```sh
...
- apiserver 支持对不同 k8s 资源，记录不同详细程度的审计日志，分为多个级别：
  ```sh
  None            # 不记录审计日志
  Metadata        # 只记录请求的元数据，比如客户端的用户名、对哪个 k8s 资源执行了什么操作
...
- apiserver 默认未启用审计日志。如果想启用，则需要在启动 apiserver 时添加参数：
  ...
  --audit-log-path=-                    # 将审计日志输出到哪个文件，填 - 则输出到 stdout
  ```
  - apiserver 输出的审计日志采用 JSON 格式。
