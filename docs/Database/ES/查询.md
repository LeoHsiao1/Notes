...
- 查询时，query 字段表示查询条件，还可添加其它字段，如下：
  ```json
  {
    ...
    "fields": [ "field1" ], // 返回的文档默认包含所有字段，可指定包含哪些字段，此时每个字段的值都放在一个 array 中
    "_source": true,        // 返回的文档默认包含 _source 字段，配置为 false 则不返回
    "_source": {            // 可指定返回的 _source 字段包含哪些字段
      "includes": [
        "filed1",
        "filed2"
      ],
    ...
    "script_fields": {      // 可执行脚本，在返回的文档中加入临时生成的字段
      "tmp_fiedl1": {
        "script": {
          "lang": "painless",
