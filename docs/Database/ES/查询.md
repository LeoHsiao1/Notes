    ...
    "query" : {
      "match" : {
        "name" : {          # 要查询的字段
          ...
          "operator": "or", # 当 query 被分词之后，几个词之间的逻辑关系。可以取值为 or 或 and
          # "fuzziness": 0,
          # "max_expansions": 50,
        }
      }
...
- 常见的几种 match 子句：
  ```sh
  "match"        : { "name" : "A B" }    # 分词之后再查询，这里是查询 name 等于 A 或 B 的文档
  ```
  ```sh
  "match_phrase" : { "name" : "A B" }    # 不分词就查询，这里是查询 name 等于 "A B" 的文档
  ```
  ```sh
  "match_all"    : { }                   # 匹配所有文档
  ```
...
- 例：
  ...
  GET /_search
  {
    ...
    "size": 2           # 最多返回的文档数。比如查询命中 100 个文档，只返回 2 个文档
  }
  ```
