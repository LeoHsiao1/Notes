...
- 对 ES 发出查询请求之后，收到的 response 格式如下：
  ```sh
...
- ES 提供了一种 DSL 查询语法，可以实现复杂的查询。
  - 它需要在请求报文 body 中填入 JSON 格式的查询参数。
  - 相关 API ：
    ```sh
...
- 查询时，query 字段表示查询条件，还可添加其它字段，如下：
  ...
  GET /student/_search
  {
    ...
    "_source": ["log*", "log.offset"], // 返回 doc["_source"] 时，只包含指定的一些子字段，可使用通配符 * ，可用 . 选择子孙字段。如果指定的字段不存在，则返回结果中不包含该字段，但不会报错
    ...
    "search_after": [...],      // 命中文档太多时，用 from 分页会占用较多内存，建议改用 search_after 分页，而不是用 scroll API 分页
  }
  ```
...
- 例：
  ...
  GET /student/_search
  {
    "query" : {
      "match" : {
        "name" : {            # 指定要查询的字段名称
          "query": "Leo"      # 指定要查询的关键词，找出哪些文档的该字段的值包含该关键词
...
- 常见的几种 match 子句：
  ...
  ```sh
  "multi_match"  : {
    "fields": ["name", "nickname", "*_name"]  # 指定多个字段进行 match 查询。不支持 fields 则会查询所有字段，实现全文搜索
    "query": "Leo",
  }
  ```
