...
- 相关 API ：
  ...
  GET   /_cat/segments/<index>?v  # 查看某个索引的所有 segment 的信息
  ...
  - 强制合并时，如果没有加上 URL 参数，则也是按照自动合并的算法，选择适合合并的 segment 。

- 例：请求 `GET /_cat/segments/test_segments?v` 的结果如下
  ```sh
  ...
  - 表中每行描述一个 segment 的信息，各列的含义如下：
    ...
    ip              # 分片所在主机的 IP
    segment         # 该 segment 在 shard 中的十六进制编号，从 _0 开始递增。新增的文档会存储在编号最大的 segment 中，但合并之后，文档所在的 segment 可能变化
    generation      # 该 segment 的十进制编号，从 0 开始递增
