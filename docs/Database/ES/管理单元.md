...
- 相关 API ：
  ```sh
  GET   /<index>                  # 查询索引的信息，包括别名、属性、配置等
  GET   /<index>/_alias           # 查询索引的别名
  GET   /<index>/_settings        # 查询索引的配置

  GET   /_cat/templates?v         # 以列表形式，列出所有索引的信息
  GET   /_resolve/index/<name>    # 解析一个名称，返回与之匹配的所有 indices、aliases、data_streams
...
：索引模板。用于在创建索引时对它进行初始化配置，比如索引的分片数、副本数。
- 新建一个索引时，会根据索引模式匹配索引模板。如果存在多个匹配的索引模板，则采用优先级最高的那个。
- 修改索引模板时，只会影响新创建的索引，不会影响已存在的索引。
- 相关 API ：
  ```sh
  PUT /_index_template/<template>   # 创建索引模板
  GET /_index_template/<template>   # 查询索引模板
