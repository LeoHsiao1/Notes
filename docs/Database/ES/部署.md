...
- 可以在 config/jvm.options 文件中配置 ES 的 JVM ：
  ...
  ```
  - JVM 内存过低时，会频繁引发 GC ，增加 CPU 负载，增加 ES 读写耗时。
  - JVM 内存最多为 32G ，否则会大幅降低 Java 对象指针的效率，增加 CPU 负载。
  - 主机应该留出一些空闲内存，相当于 JVM 内存的 0.5~1 倍，用作 Page Cache ，提高 Lucene 读写磁盘的性能。
