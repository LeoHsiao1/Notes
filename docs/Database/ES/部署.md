...
network.host: 0.0.0.0             # 该 ES 绑定 Socket 时采用的 IP
...
- 例：查询集群的健康状态
  ...
  - status 的可能取值：
    ...
    - `red` ：存在不可用的主分片。
  - ES 会将索引的各个副分片尽分散存储到不同节点上，从而实现灾备。
    - 当 ES 集群的节点数量变化时，ES 会自动迁移分片。
    - 副分片不能被分配到主分片所在节点上，因为那样不能实现灾备。如果集群内没有其它节点了，副分片就会一直处于未分配状态，导致该索引的状态为 yellow 。
