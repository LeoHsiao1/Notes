...
- v8.0
  ...
  - 默认启用了 x-pack 中的 RBAC 身份认证、TLS 加密通信。
    - 初次部署 ES 时，会自动生成自签名的 TLS 证书文件，保存到 `config/certs/` 目录下，有三个文件：
      ```sh
      http_ca.crt     # CA 证书文件，用于签署 http.p12 证书
      http.p12        # ES 提供 HTTP 端口供客户端访问，采用该证书进行 TLS 加密通信
      transport.p12   # ES 集群的各节点通过 TCP 端口相互通信，采用该证书进行 TLS 加密通信
      ```
      还会自动生成 TLS 配置参数，保存到 `config/elasticsearch.yml` 文件中。
