...
- 例：
  ...
  ```
  - ES 会将未定义映射的字段默认保存为 text 类型，并添加一个 keyword 子字段：
    ```sh
    "<field>" : {
      "type" : "text",
      "fields" : {                  # 定义子字段。子字段的取值与父字段相同，常用于映射到其它数据类型
...
- 可通过 `GET /_nodes/stats` 查询各个节点的线程池状态，例如：
  ...
  - 如果 active 的线程数达到线程池上限，即没有空闲的线程，则新增的请求会被暂存到 queue 队列，等待处理。
