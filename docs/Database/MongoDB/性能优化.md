...
- 查看存储体积：
  ...
  db.<collection>.stats().wiredTiger["block-manager"]["file size in bytes"] /1024/1024/1024   // 集合占用的磁盘空间
  ...
  db.<collection>.stats().wiredTiger["cache"]["bytes read into cache"]              // 因为读取磁盘而占用的内存体积
  db.<collection>.stats().wiredTiger["cache"]["bytes written from cache"]           // 因为写入数据而占用的内存体积
