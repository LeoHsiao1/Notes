...
- 查看存储体积：
  ```js
  db.stats().dataSize       // 数据库的数据量，包括文档、索引，单位 bytes 。这是读取到内存时的体积，也是 mongodump 不压缩时的体积
  db.stats().storageSize    // 占用的磁盘空间。可能比 dataSize 小，因为 WiredTiger 存储引擎默认进行压缩。也可能比 dataSize 大，因为删除的文档不会释放磁盘空间
  db.<collection>.stats().size        // 集合的数据量
  db.<collection>.stats().storageSize // 占用的磁盘空间
  ```
...
- projection 参数用于筛选 query 的查询结果，
  ...
  - 例：
    ```js
    db.c1.find({"name":"leo"}, {"_id": ObjectId("6264c")})
...
- 假设通过 `db.students.find({"name":"leo"})` 查询集合中的文档，默认会读取所有文档的全部字段，检查其中的 name 字段是否符合查询条件。这样会扫描全表，耗时较久。
  - 此时可以针对 name 字段创建一个索引。这会在集合文档之外存储一个独立的表，记录所有文档的 doc._id 与 doc.name 的对应关系。
  - 利用索引查询，可以避免从磁盘读取文档的全部字段，大幅减少磁盘读取量、耗时。
- 每个集合可以创建多个索引，默认有一个针对 _id 字段的索引。
  - 支持创建由多个字段组成的复合索引。
