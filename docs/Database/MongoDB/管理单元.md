...
```
- 可以创建 capped 类型的集合，这会限制集合的体积，最多存储 size KB 的文档，或者最多存储 max 条文档。
  - 当集合写满时，新插入的文档会从集合头部开始存储，覆盖旧文档。
  - 不能删除其中某个文档，只能删除整个集合。
  - 修改一条文档时，不允许增加该文档的体积。
...
- 常用的 options 参数：
  ...
  - expireAfterSeconds ：对 Date 类型的字段创建索引时，可以添加 expireAfterSeconds 参数启用 ttl 。如果一个文档过期，则自动删除该文档。
  - v ：索引的版本号。
