...
- mongo 服务器的 URI 采用以下格式：
  ...
  - 例：
    ...
    mongodb://root:******@127.0.0.1:27017/test?authSource=admin # 这是传入用户名、密码，并通过 URI 参数指定验证数据库
...
- mongodump 命令：
  ...
  mongodump
            ...
            -d <db>           # 只处理指定的一个数据库。如果不指定，则默认会导出所有数据库
  ...
  - Mongo 的 WiredTiger 存储引擎默认会将文档、索引压缩之后才存储到磁盘，压缩率大概为 50% 。假设占用磁盘为 50G ，则所有数据加载到内存中大概为 100G 。
    - 如果 mongodump 时不启用 gzip 压缩，则导出文件的体积大概为 100G 。
    - 如果 mongodump 时启用 gzip 压缩，则导出文件的体积大概为 25G 。不过该压缩文件不支持用 tar 命令解压。
    - 如果不压缩，则 mongodump 导出、mongorestore 导入的速度大概为每分钟 4G 。
  - Mongo 导出大量数据时，建议带上查询条件，分批导出数据。比如根据 oid 范围，每次导出一个月的数据。这样导出第一批数据之后，就可以开始导入，减少迁移数据的耗时。
...
：一个命令行工具，用于将一个 Mongo 的数据，在线同步到其它 Mongo ，也可同步到 Kafka 等目的端，属于异步复制。
...
- 用法：
  ...
  3. 执行命令：
      ...
      curl 127.0.0.1:9101/progress                        # 查看同步进度
      ```
...
- 副本集群（Replication）由多个 mongod server 组成，server 的角色分为三种：
  - Primary
    - ：主节点，负责处理客户端的读、写请求。
    - 副本集内，有且仅存在一个 Primary 节点，还可存在任意个（包括零个） Secondary、Arbiter 节点。
  - Secondary
    - ：从节点，负责复制主节点的数据。
    - 主节点会将所有数据库操作记录在 oplog 中，而从节点会定期从主节点复制 oplog ，执行其中的操作，从而同步主节点的数据。
    - 从节点不能处理客户端的写请求，但能处理读请求，可实现读写分离。
      - 数据库负载不大的话，没必要读写分离，客户端可以将读、写请求都发给主节点。
      - 默认从节点不支持读请求。因为从节点采用半同步复制，可能没有最新的数据。
  - Arbiter
    - ：仲裁节点。不同步数据（因此不容易出故障），仅参与投票，不能被选举。

- 与 mongod 单实例相比，副本集群能大幅提高可用性。当主节点故障时，其它节点会自动选出一个新的主节点，实现主从切换。
  - 副本集内只存在一个节点时，它会担任 Primary 节点，供客户端读写，但不能进行主从切换。
  - 副本集内有超过半数的投票节点在线时，才能开始选举。
    - 选举过程有几秒长。
    - Mongo 限制了一个副本集最多存在 7 个投票节点。
    - 与 zk 集群类似，投票节点的数量建议为奇数，为偶数时并不会提高可用性。
  - 副本集至少要部署 3 个投票节点，才能进行主从切换。
    - 建议至少部署 1 主 + 2 从节点，或者 1 主 + 1 从 + 1 仲裁节点。
    - 建议最多部署一个仲裁节点。因为部署多个仲裁节点时，可能投票选出一个尚未完全同步的从节点，担任新的主节点。

- oplog
  - ：mongod 的操作日志（operation log），存储在 local 数据库的一个集合中。
  - oplog 记录的操作具有幂等性，可能将多个写操作合并为一个写操作。
  - oplog 的体积，默认最多为磁盘空间的 5% 。
...
- 例：修改副本集的配置
  ...
  rs0:PRIMARY> rs.conf()
  {
      ...
      "writeConcernMajorityJournalDefault" : true,  // 默认为 true ，要求主节点每提交一个写操作，等同步给大多数（writeMajorityCount）节点之后，才认为写入成功，写入预写日志。属于半同步复制
      "members" : [
          {
              "_id" : 0,
              "host" : "mongo-2:27017",
