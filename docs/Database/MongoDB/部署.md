...
- mongorestore 命令：
  ...
  mongorestore <path>         # 指定要导入的 BSON 文件。也可以指定目录，这会自动发现该目录下的 bson 文件，但不会发现子目录的
            ...
            --numInsertionWorkersPerCollection 1  # 导入每个集合时，创建几个线程进行并发 insert ，默认为 1
            --gzip            # 导入时进行 gzip 解压
  ...
  - mongorestore 导入每个文档时，如果已存在相同 _id 的文档，则会报错 `duplicate key` ，跳过导入该文档。此时需要从磁盘读取 _id 索引，导致导入速度变慢。
