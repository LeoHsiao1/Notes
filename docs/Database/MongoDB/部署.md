...
- v5.0
  - 于 2021 年发布。
  - 支持创建时序集合。
  - 原本在 index build 过程中，如果 mongod 重启，则会重新开始 index build 。现在会根据重启前的进度，继续执行 index build 。
...
- v6.0
  ...
  - 移除客户端命令 mongo ，改用新命令 mongosh ，称为 mongo shell 。
- v7.0
  - 于 2023 年发布。
- v8.0
  - 于 2024 年发布。
  - 提升 30% 的读写速度。
  - bulkWrite() 原本只能修改一个集合，现在可以修改多个集合。
...
- mongodump 命令：
  ...
  - mongodump 不适合增量同步 mongod 的数据。因为在 mongodump 导出数据的过程中，mongod 可能又修改了数据。可采用以下措施：
    ...
    - 先执行一次 mongodump + mongorestore 进行全量迁移，然后执行 mongoshake ，基于 oplog 增量同步。

- mongoexport 命令：
  ```sh
  mongoexport                 # 导出为 JSON 或 CSV 格式
            -q <expression>   # mongoexport 命令兼容 mongodump 命令的大部分参数
            --fields="field1,field2"  # 只导出指定的字段。默认会导出每个文档的全部字段
            --limit <int>     # 限制导出的文档数

            --out out.json    # 导出之后保存到指定文件。默认输出到 stdout
            --type json       # 导出格式。默认为 json ，还可选 csv
...
3. 登录一个节点，启动副本集群：
    ...
    - 不允许在 priority 为 0 的节点，或仲裁节点上，进行初始化。
4. 创建账号：
    ```js
    use admin
    db.createUser(
