...
- mongorestore 命令：
  ...
  mongorestore <path>         # 指定要导入的 BSON 文件。也可以指定目录，这会自动发现该目录下的 bson 文件，但不会发现子目录的
            ...
            --numInsertionWorkersPerCollection 1  # 导入每个集合时，创建几个线程进行并发 insert ，默认为 1 。如果导入单个巨大的集合，可增加线程数
            --gzip            # 导入时进行 gzip 解压
...
- 从节点的启动流程：
  ...
  2. initial sync 阶段：从节点连接到主节点，进行一次全量同步，复制主节点的所有数据库（除了 local 数据库）到本机。此时处于 STARTUP2 状态。
