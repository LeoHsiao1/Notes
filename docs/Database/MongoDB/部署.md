...
mongodump
          ...
          --gzip                  # 导出时进行 gzip 压缩
```
- 默认会导出所有数据库，在目标目录下按数据库名分别创建子目录，为每个集合保存 `<collection>.bson` 和 `<collection>.metadata.json` 两个备份文件。
...
mongorestore
          ...
          --dryRun                # 不实际导入，只是模拟执行
          --stopOnError           # 遇到报错时立即停止导入（默认不会停止）
```
...
2. 在各个节点的配置文件中加入：
    ...
    ```
    - 启用身份认证时，客户端通过账号密码连接服务器，而各节点之间通信时采用密钥文件。
