...
- 或者用 docker-compose 部署：
  ...
  networks:
    net:
  ```
  - 可选同时部署 mongo-express ，它是一个用于管理 Mongo 的 Web 服务器，采用 Node.js 开发。
...
mongorestore
          ...
          -c <collection>
          -j 4
          --dir <directory>       # 指定要导入的目录。这会自动发现该目录下的 bson 文件，但不会发现子目录的。也可以指定一个 BSON 文件的路径
          --gzip                  # 导入时进行 gzip 解压
