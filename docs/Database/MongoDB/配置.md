...
operationProfiling:           # 慢查询日志
  mode: slowOp                # 工作模式，默认为 off 。这里采用 slowOp 模式，将执行时长超过 slowOpThresholdMs 阈值的客户端请求，记录到当前 DB 的 system.profile 集合里，该集合的最大容量为 1MB
...
- 如果已启用身份认证，则启动客户端时，需要传入用户名、密码，比如 `mongo 127.0.0.1:27017/admin -u root -p ******` 。
