...
```
- mongod 相当依赖操作系统的缓存机制。因此默认使用 50% 的主机内存，剩下的 50% 用作 Page Cache ，加速读写磁盘。
- mongod 不限制客户端连接数，每个连接最多占用 1MB 内存。
- 慢查询日志的几种模式：
  ```sh
  off       # 不记录
  all       # 记录所有数据库操作
...
- WiredTiger
  ...
  - 默认将文档存储到磁盘时，会进行压缩。而加载到内存中时，不会压缩。
  ...
  - 当 mongod 占用内存达到 cacheSizeGB 的 eviction_target（默认 80% ）时，后台的 evict 线程会开始驱逐内存中较少使用的 clean pages ，腾出内存空间。
    - 占用内存达到 eviction_trigger（默认 95% ）时，前台线程会经常阻塞，以便驱逐 clean pages 。
    - 占用内存达到 100% 时，会停止前台线程的操作。
    - 内存中的 dirty pages 达到 eviction_dirty_target（默认 5% ）时，后台的 evict 线程会开始驱逐 dirty pages ，导致丢失数据。
    - 内存中的 dirty pages 达到 eviction_dirty_trigger（默认 20% ）时，前台线程会经常阻塞，以便驱逐 dirty pages 。
