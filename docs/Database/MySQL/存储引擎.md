...
- 前缀索引（Index Prefixes）：对字符串型的字段建立索引时，可以限制只对左侧连续 n 个字符建立索引。
- 例：
  ```sql
  alter  table tb1 add index index1(name(5));
  ```
- BLOB、TEXT 类型的字段长度很大，因此只允许使用前缀索引。
- n 越小，索引体积越小。
  - n 不能过小，避免查询结果的精度过低，查询一条数据却返回多条数据。
  - 一般的 InnoDB 表，n 最大取值为 767 bytes 。
  - 如果查询语句中的值，长度超过前缀索引的 n ，比如 `where name like 'one%'` 。则先查询索引，找到一批前缀匹配的数据，再从原表读取这些数据的完整值，进行查询。

### 全文索引

- 可以对 char、varchar、text 类型的字段建立全文索引（fulltext index），方便在大型文本中搜索一个字符串。
  - 全文索引采用倒排结构，而不是 B+ tree 。
  - 全文索引不支持前缀索引。
- 例：
  ```sql
  create fulltext index index1 on tb1(comment);  -- 对 tb1 表的 comment 字段建立一个全文索引
  ```
...
- 例：tb1 表中 id 字段为主键并建立了索引，分析查询过程如下
  ...
  - type ：查询类型。查询速度从高到低依次分为：
    ...
    index_merge # 使用了两个或更多索引，最后合并它们的查询结果。where 子句用 and 则取交集，用 or 则取并集
