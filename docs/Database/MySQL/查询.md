...
- SELECT 语句用于获取、计算一个值，默认打印到终端，像 Linux 的 echo 命令。例如：
  ...
  ```
- SELECT 语句可以独立执行，也可以附加 FROM、WHERE、order by、limit 等子句（clause），实现多种语法功能。

### AS

- 用 AS 关键字，可以为 SELECT 获取的一个值、一个字段增加别名。
- 例：
  ```sql
...
- `SELECT <field>... FROM <table>...` 默认会返回数据表中的每条数据的指定字段。可加上 WHERE 子句，只返回匹配查询条件的那些条数据的字段。
...
- SELECT 返回的数据默认是按 B+ tree 中存储位置排序的，可加上 ORDER BY 子句，按指定字段排序。
- 例：
  ```sql
...
- SELECT 查询到一些数据时，默认会全部返回。可加上 LIMIT 子句，只返回 n 条数据。
- 语法：
  ```sql
