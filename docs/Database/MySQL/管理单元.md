...
- select 语句用于获取、计算一个数据，默认打印到终端，像 Linux 的 echo 命令。例如：
  ...
  -- 可进行 C 语言的算术运算
  select (1 + 2 - 3)*4;
  select 10/3;              -- 除法运算，结果为 3.3333
  select 10%3;              -- 取模运算，结果为 1
  select round(3.5);        -- 可用 round() 函数取整，进行四舍五入。这里结果为 4
  select round(10/3);       -- 结果为 3
  ```
  ```sql
  mysql> select 10/3 > 3;   -- 比较运算的结果为 1、0 ，表示 true、false
  +----------+
  | 10/3 > 3 |
  +----------+
  |        1 |
...
insert into tb1 (name, date) values ('one', '2019-12-01');  -- 插入一行数据，只给部分字段赋值，其它字段会采用默认值
```
