...
1. 在 master 的配置文件中加入：
    ...
    # replicate-do-db=db1        # 让 slave 同步指定的数据库（其它的默认不同步）
    # replicate-do-db=db2
    # replicate-ignore-db=db1    # 不让 slave 同步指定的数据库（其它的默认同步）
...
4. 启动 slave ，登录后执行以下命令：
    ...
    change master to                          -- 指定 master 节点
        ...
        master_deply=0;                       -- 同步延迟，让 slave 只执行 n 秒之前的事务
    start slave;                              -- 开始与 master 同步
    show slave status;                        -- 显示 slave 的状态

    -- stop slave;                            -- 停止同步
    -- reset slave;                           -- 删除所有中继日志，重新开始同步
    ```

如果 slave 落后 master 的数据较多，建议先暂停 master 的写入：
1. 登录 master ，切换到要同步的数据库，执行 `flush tables with read lock;` ，清空数据表的写缓存，并加上只读锁。
2. 用 mysqldump 等方式导出 master 的数据，导入到 slave 中。
