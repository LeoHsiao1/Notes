...
MySQL 备份数据的几种方式：
- 先使用 mysqldump 等命令从某个 MySQL 导出数据，再导入另一个 MySQL 。
  - 优点：适合离线备份，备份速度快。
  - 缺点：导出时，需要 MySQL 停止写入，以免遗漏数据。导入时，需要 MySQL 停止写入，以免数据冲突。
- 给 MySQL 启用主从同步。
  - 优点：适合实时备份，不需要 MySQL 停止写入。
  - 缺点：需要两个 MySQL 保持网络连通，备份速度受网速影响。
