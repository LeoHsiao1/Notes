...
- MySQL
  ...
  - v8.0
    ...
    - 实现 DDL 操作的原子性。
    - 密码的存储方式从 mysql_native_password 改为 caching_sha2_password 。老版本的客户端可能不兼容，导致不能登录。可以修改用户的密码类型：
      ```sh
...
- MHA（Master High Availability）：一个实现高可用的工具。
- [官方文档](https://github.com/yoshinorim/mha4mysql-manager/wiki)
- 原理：
  - 当它发现主从集群的 master 节点故障时，会自动进行主从切换：修改 slave 节点的配置，将它改为 master 节点。
- 优点：
  - 手动主从切换需要几十分钟，而自动主从切换只需要几十秒。
- 缺点：
  - 主从切换之后， master 节点的 IP 变化，需要让客户端连接到新 IP 的 master 节点。
    - 解决办法是创建一个 VIP ，或部署一个 mysql proxy ，反向代理 MySQL 。
...
- 缺点：MySQL 节点之间同步数据的方式只是复制事务，不能保证分布式集群的一致性。
  - 例如两个 MySQL 同时写入同一个表时容易主键重复、同时修改同一个表时容易同步失败。因此主主复制集群一般不可用。
...
- 传统的 MySQL 集群大多采用主从方案，与之相比，MGR 集群的优缺点如下：
  - 优点：
