...
- Cluster 集群
  - 由 MySQL 官方发布。
  - 主从集群的主要作用：数据备份、读写分离。
  - 分布式集群的主要作用：将应用部署成多个实例，一个发生故障时可以立即换用另一个，避免单点故障，实现高可用。
...
进行主从同步之前，应该先让 slave 与 master 保持一致（数据相同、配置相同），否则以后 slave 进行同步时可能出错。做法如下：
...
- ：一个实现高可用的工具，能监控主从集群的 MySQL 服务器，自动进行主从切换。
