...
- 例：从 MySQL 获取 binlog ，写入 Kafka
  ...
  docker run -d \
      ...
      -e canal.instance.filter.regex=db1\\.test.* \     # 获取 binlog 时，筛选 db1 数据库中，匹配 test.* 的所有表
      # -e canal.instance.master.timestamp=xx \         # 获取从指定时间戳开始的 binlog 。如果不指定，则获取从当前时刻开始的 binlog
  ...
  tail -f canal-server/logs/example/*.log
  ```
...
- 组复制（MySQL Group Replication ，MGR）：是 MySQL 5.7 引入的插件，用于部署分布式的 MySQL 集群。
