...
- 法一：在 shell 终端中，用 mysqldump 命令导出数据库，保存为 sql 脚本。如下：
  ```sh
  mysqldump
            -h <ip> -u <user> -p
            -A            > dump.sql    # 导出所有数据库
  ...
  ```
  - 执行该 sql 脚本中的 SQL 命令，即可重建数据库并导入数据。
  - 导出的 sql 脚本可以再用 zip 压缩，压缩率大概为 30% 。

- 法二：用 mysql 命令进入 MySQL 客户端的终端，然后导出数据库，保存为文本文件。如下是导出 csv 格式的文件：
  ```sql
  use db1;
...
- 法一：在 shell 终端中，直接导入 sql 脚本：
  ...
  ```
  - 建议先执行 `set session sql_log_bin = off;` ，在导入时暂时禁用 binlog 。
...
- 法三：用 mysql 命令进入 MySQL 客户端的终端，然后从文本文件导入数据。如下是导入 csv 格式的文件：
  ```sql
