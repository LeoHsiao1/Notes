...
- 异步复制
  ...
  - MySQL 主从集群默认采用异步复制，使用半同步复制则需要安装插件。
- 同步复制
...
3. 在 slave 的配置文件中加入：
    ...
    # slave_skip_errors = off   # 如果 slave 执行复制的事务出错，是否跳过。取值如下：
            ...
            # all ：跳过所有错误。此时 slave 会继续复制，但依然会打印报错日志，与 master 的差异可能越来越大
...
4. 启动 slave ，登录后执行以下命令：
    ...
    change master to                          -- 指定 master 节点
        ...
        master_password='******',
        master_log_file='mysql-bin.000003',   -- 从 master 的哪个 binlog 文件开始复制。显示 master 的状态可以知道当前的 binlog 编号
        master_log_pos=576,                   -- 从 master_log_file 的哪个位置开始复制
