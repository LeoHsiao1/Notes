...
- MySQL 在执行某些 SQL 时会自动创建内部临时表，比如 group by 。
  ...
  - 如果临时表超出内存限制，则会保存到磁盘 tmpdir 目录的临时文件中，改用 InnoDB 引擎。
...
- 相关命令：
  ...
  select event_name,current_alloc from sys.memory_global_by_current_bytes limit 10; -- 查询各种事件占用的内存
  select thread_id,user,current_allocated from sys.memory_by_thread_by_current_bytes limit 10;  -- 查询各个客户端占用的内存
  ```
