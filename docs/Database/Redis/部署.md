...
- v5.0
  ...
  - 为了政治正确，将 slave 改名为 replica 。
  - 优化了删除策略，更快删除已过期的 key 。

- v6.0
  - 于 2020 年发布。
  - 支持 SSL 连接。
  - 增加了访问控制列表（ACL），可以定义多个用户，分配不同的权限。
  - 采用多线程网络 IO 。将读写 socket 等操作，交给后台线程执行，避免阻塞主线程。
  - 发布了集群代理 Redis Cluster proxy 。

- v7.0
  - 于 2022 年发布。
  - 配置 `appendonly yes` 时，会将单个 aof 文件，拆分成多个小型文件，存储在 `appenddirname "appendonlydir"` 目录下。
    - 默认配置了 `aof-use-rdb-preamble yes` ，因此除了 aof 目录，还会定期生成一个 dump.rdb 文件。但 redis 重启时，会优先加载 aof 目录。
    - 可以配置 `save ""` ，禁止生成 dump.rdb 文件。

- v8.0
  - 于 2025 年发布。
  - 优化了架构，大幅提升性能。
  - 新增了几种数据结构：vector set、JSON、time series、Bloom filter
