...
- 可以执行以下命令，立即将数据保存到 dump.rdb 文件中：
  ...
  ```
- [redis-port](https://github.com/CodisLabs/redis-port) ：一个命令行工具，支持导出、导入、解析 dump.rdb 文件。
...
- 采用 RDB 或 AOF 模式时，如果要恢复数据，只需将 dump.rdb 或 appendonly.aof 文件拷贝到 Redis 工作目录下，然后重启 Redis 。
  - Redis 启动时，默认会读取 dump.rdb ，将其中的数据载入内存。
    - 如果配置了 `appendonly yes` ，则 Redis 不会读取 dump.rdb ，而是读取 appendonly.aof 。
...
- 原理：
  ...
  2. replica 作为客户端连接到 master ，执行 `PSYNC <replicationid> <offset>` 命令，复制 master 执行过的命令，在 replica 处执行这些命令。
      ...
      - 复制命令的方式适合增量同步。如果 master 发现 replica 的差异过大，则会拷贝全部数据的快照，进行一次全量同步。
