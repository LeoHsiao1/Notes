...
- 采用 RDB 模式时，如果要恢复数据，只需将 dump.rdb 文件拷贝到 Redis 工作目录下，然后重启 Redis 。
  - Redis 启动时，会自动读取 dump.rdb 文件，将其中的数据载入内存。
...
- 采用 AOF 模式时，如果要恢复数据，只需将 appendonly.aof 文件拷贝到 Redis 工作目录下，然后重启 Redis 。
  - 如果配置了 `appendonly yes` ，则 Redis 启动时不会读取 dump.rdb ，而是读取 appendonly.aof 。
  - aof 文件的开头是 RDB 格式的内容，相当于全量备份。后面记录了执行的每条命令，相当于增量备份。
