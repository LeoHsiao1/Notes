...
- 创建一个 ISM 策略之后，其工作流程如下：
  1. 第一次执行：初始化 ISM 策略，让索引进入初始状态。然后暂停执行该 ISM 策略，等待下一次执行。
  2. 下一次执行：执行索引当前状态的动作，切换到下一个状态。然后暂停执行。
  3. 下一次执行：重复第 2 步。
...
- 例：创建一个 ISM 策略
  ...
  ```
  - 创建 ISM 策略之后，会根据 index_patterns 自动绑定新建的索引，但不会影响已存在的索引，需要手动绑定。
