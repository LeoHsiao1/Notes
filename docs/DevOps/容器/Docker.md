...
```
- 如果将宿主机的某个目录、文件或数据卷，挂载到容器内的某个目录，则容器被删除之后该目录也会一直保存在宿主机上，从而持久保存数据。
- 挂载宿主机的目录、文件时：
  - 如果挂载的宿主机目录不存在，则会自动创建它再挂载。
    - 但如果挂载的是一个文件，且该文件路径不存在，比如 `-v /root/f1:/root` ，则会在宿主机上创建一个路径为 `/root/f1` 的目录再挂载，可能导致容器启动失败。
  - 如果容器内应用以非 root 用户启动，则对于挂载的目录、文件可能没有访问权限，此时需要先在宿主机上修改其文件权限再挂载。
    - 挂载数据卷时，会自动创建合适权限的 _data 目录再挂载，因此不会出现该问题。
- 挂载数据卷时：
