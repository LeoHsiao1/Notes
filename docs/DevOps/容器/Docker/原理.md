...
- Docker 采用 C/S 架构工作：
  ...
  - 用户可以在宿主机上执行 docker 命令，作为客户端，与 dockerd 交互，比如请求启动容器。
    ...
    - 非 root 用户无权访问 docker.sock 文件，导致无权执行 docker ps 等命令。此时可以将该用户加入 docker 用户组：`sudo usermod leo -aG docker` ，从而开通权限。
...
- subsystem
  ...
  - 每种 subsystem 通过读写一组文件来实现配置，例如：
    ...
    memory.usage_in_bytes # 记录进程当前的内存使用量，包括 RSS 和 Page Cache 。因此，如果 Docker 容器进行大量磁盘 IO ，则监控到的内存开销会比 RSS 偏大
    memory.oom_control    # 取值为 0 时，启用 OOM-killer ，当进程占用的内存超过限制时杀死它。取值为 1 时，禁用 OOM-killer ，只是暂停进程
    ```
