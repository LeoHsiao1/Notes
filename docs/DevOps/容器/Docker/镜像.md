...
docker
      ...
      login harbor.test.com         # 登录一个镜像仓库。有的仓库允许未登录用户 pull ，但登录之后才允许 push
          -u <username>
          -p <password>

      tag <image>[:tag] <image>[:tag] # 给镜像添加另一个名称和标签
```
- 例：
  ```sh
...
- 例：拉取镜像的过程
  ...
  1.20: Pulling from library/nginx            # 开始从 docker hub 下载
  e5ae68f74026: Pull complete                 # 下载镜像中的一个 layer ，如果本机已存在则不会下载
...
- 可以将宿主机上存储的 docker 镜像，推送到镜像仓库服务器进行存储。
  - 默认采用官方镜像仓库 docker.io ，用户也可以自己部署仓库服务器。
...
- Docker 镜像在宿主机上存储为一些零散的文件，可以用以下命令导出：
  ...
  docker save <image>... | gzip > images.tgz  # 打包并压缩，大概压缩到 40% 大小
