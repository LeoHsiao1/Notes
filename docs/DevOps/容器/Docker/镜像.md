...
- `<image_id>` ：取自其十六进制哈希值的开头 n 位。
  - 因为未指定完整的哈希值，只能用于在本机标识镜像，不能用于 pull、push 时标识镜像。
- `<image_name>@<digest>` ：镜像名加完整的哈希值。
...
- `<image_name>[:tag]` ：镜像名加标签，由用户自定义。
  ...
  - 镜像名（image_name）是一个字符串。可以包含 / 字符，用于划分多个层级。例如：
    ```sh
    nginx:1.23
  ...
  - 标签（tag）用于区分同一 image_name 的镜像的不同版本。例如：
    ...
    nginx:1.24
    ```
  - image_tag 的取值通常来自 git tag 或日期。
  - 省略 image_tag 时，默认相当于指定 image_tag 为 latest 。
...
- image index 中每个镜像都有一份镜像清单（image manifest），记录该镜像的简要信息。例如：
  ```json
  {
  ...
  ```
  - 根据清单中的 size、digest ，客户端就能检查镜像的 config、layers 是否正确。
