...
- k8s 有两种监控 node 状态的方式：
  ...
  - 每个 node 的 kubelet 会在 kube-node-lease 命名空间创建一个 Lease 对象，并定期发送一次请求到 apiserver ，更新 Lease 对象的 renewTime 时间戳，从而证明 node 在线。

- k8s Lease 对象表示租约，常见用途：
  - 心跳检查
    - 例如每个 node 的 kubelet 会定期更新 Lease 对象的 renewTime 时间戳。
  - 分布式锁
  - 分布式选举
    - 例如 kube-controller-manager 运行了多个实例，共用 kube-system 命名空间中一个名为 kube-controller-manager 的 Lease 对象。
      - 选举流程：每个实例同时尝试更新该 Lease 对象的 renewTime 时间戳。只有一个实例能成功更新，开始持有该 Lease 对象，担任分布式集群的 leader 角色。其它实例会因为 Lease 对象的 resourceVersion 已变化而更新失败，于是放弃更新，担任 follower 角色。
      - leader 持有 Lease 对象时，需要定期更新 renewTime 时间戳，该操作称为续约（renew a lease）。如果 follower 发现 Lease 对象的 renewTime 时间戳超过 leaseDurationSeconds 时长未更新，则认为 leader 故障，开始选举新的 leader 。
    - 同理，kube-scheduler 也运行了多个实例，通过 Lease 对象进行分布式选举。配置示例：
      ```yml
      ...
      spec:
        ...
        holderIdentity: node-1_632ad4a5-386e-4a41-97ba-7ac54ad414fc   # 该 Lease 对象目前被谁持有，这里填的是 kube-scheduler 实例的名称
        leaseDurationSeconds: 15                        # 该 Lease 对象的续租时限
        leaseTransitions: 12                            # 该 Lease 对象被转手了多少次，每次选举出新 leader ，会将该值加 1
        renewTime: "2022-08-12T03:17:47.583942Z"        # 该 Lease 对象最后一次被更新的时刻
      ```
    - leader 没故障时，也可能触发选举。例如：
      - apiserver 响应时长超过 leaseDurationSeconds ，导致 leader 一直不能更新 renewTime 。
      - leader 与 follower 的时钟误差超过 leaseDurationSeconds 。
