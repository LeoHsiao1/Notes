...
- Operator ：一种部署 k8s 应用的方案。将应用注册为 CRD 资源，再添加 Custom Controller 来自动管理它。
  - 使用 Operator 能自动完成大部分工作。比如部署一个数据库服务器时，自动配置 StatefulSet、Service、Volume 。部署之后，自动创建 Job 来升级数据库、备份数据。
