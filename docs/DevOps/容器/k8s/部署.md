    ...
    services:                                   # 配置各个 k8s 组件
      ...
      kubelet:
        ...
        extra_args:
          ...
          serialize-image-pulls: "false"        # 是否同时只拉取一个镜像，默认为 true

          # 设置每个 Node 保留的资源，其它资源才可分配给 pod
          enforce-node-allocatable: pods,kube-reserved,system-reserved
          kube-reserved: cpu=0.5,memory=1Gi                                   # 保留给 kubelet、容器运行时等 k8s 系统进程
          kube-reserved-cgroup: /system.slice/kubelet.service                 # 这里需要手动创建 mkdir -p /sys/fs/cgroup/cpuset/system.slice/kubelet.service
          system-reserved: cpu=0.5,memory=1Gi,ephemeral-storage=10Gi,pid=1000 # 保留给 sshd 等系统进程
          system-reserved-cgroup: /system.slice

          eviction-hard: memory.available<10%,nodefs.available<10%,imagefs.available<10%,nodefs.inodesFree<10%  # 硬驱逐阈值
          # eviction-soft: memory.available<15%,nodefs.available<15%  # 软驱逐阈值
