...
kubectl
        ...
        exec <pod> -- <command>     # 在 pod 中执行命令。注意在 command 之前加上分隔符 --
            ...
            -it                     # 进入容器内终端
        rollout                     # 滚动更新应用，仅限 daemonset、deployment、statefulset 类型
            restart <resource> [name]...  # 根据 spec.strategy 重启 pod （在后台异步执行），如果省略 name 则重启所有
            undo <resource> <name>  # 回滚到之前部署的版本
                --to-revision=0     # 回滚到指定编号的版本。默认为 0 ，即当前版本
