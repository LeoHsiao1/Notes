...
- 查询到的数据主要分为以下几种形式：
  ...
  - Table
  - Heatmap
- Graph 类型的 Panel ，默认最多显示 1470 个数据点。
  - 查看 Prometheus 指标时，如果时间尺度很大、包含很多数据点，则采用固定的时间间隔（比如 `[1m]`）可能会遗漏一些数据点不能显示，建议改用 Grafana 内置变量 `[$__interval]` ，它会根据时间尺度自动调整时间间隔。例如查看 1d 时长的指标， __interval 变量会取值为 1d/14710≈1m 。
