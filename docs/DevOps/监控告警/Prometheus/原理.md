...
- 根据数据类型，对 metrics 分类：
  ...
  - 瞬时矢量（instant vector）
    ...
    - 一个 time series 在每个时刻只有一个 sample ，而一个 vector 在每个时刻有一组 sample 。
  - 范围矢量（range vector）
    - ：instant vector 的每个 sample 只包含一对 timestamp、value ，而 range vector 的每个 sample 包含附近一段时间范围内的多对 timestamp、value 。
    - 例如 `go_goroutines{job="prometheus"}[1m]` 属于范围矢量，先从 TSDB 中读取 instant vector 形式的 sample ，然后转换成 range vector 形式的 sample 。其中单个 sample 如下：
      ```json
      {
          "metric": {
...
- Prometheus 采集到一批 metrics 数据时，首先暂存在内存中。同时将这些数据保存到磁盘的 wal 目录下，从而避免 Prometheus 终止时丢失内存中的数据。
...
- Prometheus 会将采集到的每行 metric 文本，转换成一种数据结构：采样点（sample），又称为数据点（point）。这样便于写入 TSDB 时序数据库。
  - 例如 `go_goroutines{instance="10.0.0.1:9090", job="prometheus"}   80` 这行 metric ，会转换成一个 sample ：
    ```json
    {
  ...
  - 同一条指标的多个 sample 具有相同的 label-value set ，因此每个 sample 只需记录时间戳、值，如下：
    ```golang
    type sample struct {
        timestamp  int64
        value  float64
...
- 对于采集到的每个 time series ，Prometheus 会在内存中分别创建一个数据结构 memSeries 来保存数据。如下：
  ...
  - 对于每个 time series ，计算其 label-value set 的哈希值，记作 seriesId 。
    ...
    - Prometheus 采集、查询时涉及的 seriesId 越多，则基数越大，在内存中创建的 memSeries 越多，占用的 CPU、内存越多。
  - 一个 time series 包含多个 sample ，保存在同一个 memSeries 中。该 memSeries 会保存一份 label-value set ，被这些 sample 共享，然后在 chunk 空间中保存各个 sample 的 timestamp、value 。
  - 新采集一个 sample 时，
...
- 每个 time series 通常每隔 scrape_interval 时长采集一个 sample ，因此每个 memSeries 中保存的 sample 逐渐增多，占用内存也增多，那么什么时候写入磁盘呢？
...
- `data/chunks_head/` 目录下保存了多个数据文件，每个文件的最大体积为 128MB 。
  ...
  - 这些文件会保存 2 小时以上。每隔 2 小时，Prometheus 会将开头 2 小时范围内的一连串文件，压缩为一个新的 block 目录，从而持久化保存。并更新 wal 目录的 checkpoint 。

### block

- 每个 block 目录用于持久化保存至少 2 小时范围的数据。
  - `${block}/chunks/` 目录用于保存压缩之后的数据，分为多个文件，文件名是从 0 开始递增的编号。每个文件的最大体积为 512MB 。
...
- 因为顺序读写磁盘的速度比随机读写快多倍，Prometheus 一般不会修改 block ，只允许读取。
  ...
  - 当用户请求删除某些 metrics 数据时，Prometheus 不会立即修改 block 的数据文件，而是在 tombstones 文件中标记哪些数据待删除。等下一次压缩、合并 block 时，才从磁盘删除这些数据。
...
- 每个 block 目录下分别建立了一个 index 索引文件，记录以下信息：
  - 正排索引：记录该 block 存储的所有 time series ，每个 time series 记录以下信息：
    ...
    - chunks ：该 time series 的 sample 数据存储在 `${block}/chunks/` 目录下的哪些文件中。
