...
- 用户查询 Prometheus 时，通常是发送以下 HTTP 请求：
  ...
  - 实际上：用户查询的 time 时刻处，不一定刚好采集了 sample 。为了方便用户查询，Prometheus 采用 lookback 机制：默认取过去 5 分钟，即时间范围 `[time-5m, time]` 内的最后一个 sample ，当作 time 时刻的 sample 。如果该时间范围内连一个 sample 都不存在，则查询结果为空，此时称该 time series 已过时（stale），在最近没有采集 sample 。
  - 以下是一个查询示例：
    ```sh
