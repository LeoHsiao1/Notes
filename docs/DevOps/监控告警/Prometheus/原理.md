...
- Prometheus 会将采集到的每行 metric 文本，转换成一种数据结构：采样点（sample），又称为数据点（point）。这样便于写入 TSDB 时序数据库。
  - 例如 `go_goroutines{instance="10.0.0.1:9090", job="prometheus"}   80` 这行 metric ，会转换成一个 point ：
    ```json
    {
  ...
  - 同一条指标的多个 point 具有相同的 label-value set ，因此每个 point 只需记录时间戳、值，如下：
    ```golang
    type sample struct {
        timestamp  int64
        value  float64
...
- 对于采集到的每个 time series ，Prometheus 会在内存中分别创建一个数据结构 memSeries 来保存数据。如下：
  ...
  - 一个 time series 包含多个 point ，保存在同一个 memSeries 中。该 memSeries 会保存一份 label-value set ，被这些 point 共享，然后在 chunk 空间中保存各个 point 的 timestamp、value 。
  - 新采集一个 point 时，
...
- 每个 time series 通常每隔 scrape_interval 时长采集一个 point ，因此每个 memSeries 中保存的 point 逐渐增多，占用内存也增多，那么什么时候写入磁盘呢？
  - 每个 memSeries 会创建多个 chunk 空间，每个 chunk 用于保存大约 120 个 point 。
...
- 每个 block 目录下分别建立了一个 index 索引文件，记录以下信息：
  - 正排索引：记录该 block 存储的所有 time series ，每个 time series 记录以下信息：
    ...
    - chunks ：该 time series 的 point 数据存储在 `${block}/chunks/` 目录下的哪些文件中。
