...
- 根据数据结构，对 metrics 分类：
  ...
  - 瞬时矢量（instant vector）
    ...
    - 一个 time series 在每个采样时刻只有一个 point ，而一个 vector 在每个采样时刻有一组 point 。
  - 范围矢量（range vector）
    - ：instant vector 的每个 point 只包含一对 timestamp、value ，而 range vector 的每个 point 包含一段时间范围内的多对 timestamp、value 。
    - Prometheus 最初将这种 vector 称为 matrix ，后来改名为 range vector 。
    - 例如 `go_goroutines{job="prometheus"}[1m]` 属于 range vector ，先从 TSDB 中读取 instant vector 格式的 point ，然后转换成 range vector 格式的 point 。其中单个 point 如下：
      ```json
      {
          "metric": {
...
- 用户可在 Prometheus 的 Web 页面执行 PromQL 查询表达式，可显示成两种图表：
  - Table
    ...
    - 此时前端会向 Prometheus 发出 /api/v1/query 请求，返回 instant vector 格式的 points 。
  - Graph
    - ：查询某段时间范围内的所有 points ，然后以时间为 x 轴显示成曲线图。
    - 此时前端会向 Prometheus 发出 /api/v1/query_range 请求，返回 range vector 格式的 points 。
...
- 关于 StaleNaN 标记。
  ...
  - 当用户查询 instant vector 时，如果过去 5 分钟内的最后一个 point 的值为 StaleNaN ，则放弃 lookback ，返回的查询结果为空。
