...
- 根据数据结构，对 metrics 分类：
  ...
  - 范围矢量（range vector）
    ...
    - Prometheus 最初将这种 vector 称为 matrix ，后来改名为 range vector 。
    - 例如 `go_goroutines{job="prometheus"}[1m]` 属于 range vector ，先从 TSDB 中读取 instant vector 格式的 sample ，然后转换成 range vector 格式的 sample 。其中单个 sample 如下：
      ```json
      {
          "metric": {
...
- 一个 Series 包含多个 sample ，它们具有相同的 label-value set 。为了节省存储空间，只记录一份 label-value set ，再给每个 sample 记录一份时间戳、取值。数据结构如下：
  ```golang
  ...
  type Series struct {
      ...
      Floats     []FPoint       // 该 Series 的 float 类型的 sample 数组
      Histograms []HPoint
  }
  type FPoint struct {
  ...
  ```
  - samples 取值通常是 float 类型，但也有 histogram 类型的，用于绘制直方图。
  - 一连串 sample 的 timestamp 通常前缀相同，存储到磁盘时还可以压缩。
...
- 用户可在 Prometheus 的 Web 页面执行 PromQL 查询表达式，可显示成两种图表：
  - Table
    ...
    - 此时前端会向 Prometheus 发出 /api/v1/query 请求，返回 instant vector 格式的 samples 。
  - Graph
    - ：查询某段时间范围内的所有 samples ，然后以时间为 x 轴显示成曲线图。
    - 此时前端会向 Prometheus 发出 /api/v1/query_range 请求，返回 range vector 格式的 samples 。
    - 显示曲线图需要加载很多 samples ，开销大，可能导致网页卡顿。
...
- Prometheus 是离散采样，比如每隔 30s 采集一次监控指标，这本就存在测量误差。另外 Prometheus 在执行查询表达式时，还可能增加误差。
- 如果用户想完全消除监控的误差，比如电商场景，则建议记录所有事件的日志，然后搭建基于日志的监控系统，比如 ELK 。
...
- 用户查询 Prometheus 时，通常是发送以下 HTTP 请求：
  ...
  ```
  - 理论上：用户查询 time 时刻处的监控指标，于是 Prometheus 将查询表达式转换成 label-value set ，从 TSDB 读取 samples 。
  - 实际上：用户查询的 time 时刻处，不一定刚好采集了 sample 。为了方便用户查询，Prometheus 采用 lookback 机制：取过去 `--query.lookback-delta=5m` 分钟，即时间范围 `[time-5m, time]` 内的最后一个 sample ，当作 time 时刻的 sample 。如果该时间范围内连一个 sample 都不存在，则查询结果为空，此时称该 time series 已过时（stale），在最近没有采集 sample 。
  - 以下是一个查询示例：
    ```sh
            ...
            "result": [
                {
                    ...
                    "value": [
                        # TSDB 中存储的 sample.timestamp 不一定等于 query time ，但 Prometheus 返回查询结果时，会自动修改 sample.timestamp ，对齐 query time
                        1589241600,
                        "7"
                    ]
                }
    ...
    ```
  - 如果用户查询的是 range vector ，则会发送 `/api/v1/query_range` 请求，不会采用 lookback 机制。

- 评价 lookback 机制：
  - 优点：
    - 用户不必准确指定 time ，就能查询到 sample 。
  - 缺点：
    - 增加了时间误差，用户查询到的 sample 可能是几分钟之前的值。降低 scrape_interval 和 eval_interval 可减小该误差。
    - 当 Prometheus 停止采集一个 time series 之后（比如 target 下线），用户以最新时刻去查询，依然能查询到 sample ，造成该 time series 依然被采集的假象。
...
- 关于 StaleNaN 标记。
  - Prometheus 定义了以下常量：
    ...
    ```
  - 如果 Prometheus 上一次 scrape 采集到了某个 time series 的 sample ，下一次 scrape 却没采集到该 time series ，则给该 time series 追加一个取值为 StaleNaN 的 sample ，表示该 time series 已过时。
  - 当用户查询 instant vector 时，如果过去 5 分钟内的最后一个 sample 的值为 StaleNaN ，则放弃 lookback ，返回的查询结果为空。
  - 当用户查询 range vector 时，会忽略取值为 StaleNaN 的 sample ，不包含在查询结果中。
...
- 下面分析 Prometheus 的 [extrapolation()](https://github.com/prometheus/prometheus/blob/main/promql/functions.go) 函数源码：
  ```golang
  func extrapolatedRate(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper, isCounter, isRate bool) Vector {
      ...
      var (
          samples    = vals[0].(Matrix)[0]      // range 时长内采样的全部数据点，保存为一个数组
