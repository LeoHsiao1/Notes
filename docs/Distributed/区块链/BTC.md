...
- 区块 body 中可以记录 n≥1 个交易（transaction），最大数量取决于区块的容量限制。
...
- 一个交易记录成特定格式的字节数据，像账本中的一条记录，主要包含以下信息：
  ...
  - outputs ：包含 n≥1 个输出项。
    - 每个输出项的主要内容：
      ...
      - value ：输出的 BTC 数量。
      - pkScript
    - 一个交易的总输入减去总输出的差值，会被矿工视作交易费拿走，即 `fee = sum(inputs) – sum(outputs)` 。
  - locktime ：表示允许将该交易打包到区块中的最早时间，占 4 bytes 。
    - 如果取值小于 5 亿，则视作区块高度。
    - 如果取值大于 5 亿，则视作 Unix 时间戳。
    - 交易通常将 locktime 设置为 0x00000000 ，表示不限制。

- 一个交易数据的体积没有上限。但增加体积时，通常需要增加交易费，吸引矿工打包该交易。
...
- 如果一个账户拥有一些 BTC ，则可以发起一个交易，输入一些 BTC ，然后输出给其它账户。
  ...
  - 如果一个交易输出的 BTC ，尚未被目标账户花费，则称为未花费交易输出（Unspent TX Output，UTXO）。
    ...
    - 一个交易的 UTXO 必须被一次性全部花费。例如 UTXO 为 5 BTC 时，用户可以新建一个交易，发送 1 BTC 给别人，然后将剩下的 BTC 发送给自己。
...
- Null Data（空数据）
  - ：pkScript 的开头是一个操作符 OP_RETURN ，表示终止脚本执行。
    ...
    - 该交易的 UTXO 不能被花费。
  - 常用于将用户自定义的数据保存到区块链中。此时将其 UTXO 设置为 0 ，只需支付交易费。
...
- 节点分类：
  ...
  - 瘦节点（Thin Node）
    ...
    - 全节点能够自己验证一个交易、区块是否有效，而瘦节点需要依赖全节点。
...
- 一个完全节点的启动步骤：
  1. 发现其它完全节点，建立对等连接。
      - 通常在本机存储了一些完全节点的 IP 地址，可在启动时快速连接它们。
      - 默认监听 TCP 8333 端口，供其它节点连接自己。也可以不监听，只允许自己主动连接其它节点。
  2. 从其它完全节点下载区块数据，更新本机存储的区块链。
...
- 矿工挖矿的流程：
  1. 接收网络上广播的多个交易，打包成一个区块。
      - 出于利益，矿工会优先打包交易费高、体积小的交易。
      - 矿工需要验证交易是否有效，比如 sigScript、locktime 。
  2. 尝试穷举区块的 nonce 值，直到满足挖矿难度，有权打包该区块。
      - 如果发现其他矿工抢先打包了新区块，则放弃打包当前区块，开始下一个区块。
