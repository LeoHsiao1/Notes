...
- 以太坊升级到 v2 分为三个阶段：
  ...
  3. 创建 n=64 条分片链（Shard Chain），将以太坊数据分散存储到分片链上。
      - 这样方便横向扩容，将 TPS 提高 n 倍。
      - 这会将全网的验证者随机分成 n 组，分别处理一条分片链。每条分片链都需要 16384 个验证者。
        - 当分片链的区块被打包、排序，并得到超过 2/3 的验证者投票认同之后，就会保存到信标链。
        - 缺点：单个分片链的验证者较少，容易受到 51% 攻击。
