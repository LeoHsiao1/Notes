...
- ERC721 是2018 年发布的一种基于以太坊的代币标准，称为不可替换代币（Non-Fungible Token，NFT）、非同质化代币。
...
- layer2 ：在区块链的基础上，增加一层应用供用户访问。
  - 优点：layer1 提供的功能有限，而 layer2 能执行功能复杂的操作，只需将交易结果保存到区块链，以保证安全性。
...
- 零知识汇总（Zero-knowledge rollup）
  ...
  - 缺点：零知识证明比较复杂，难以兼容 EVM ，还需要进行大量 EVM 计算、消耗大量 gas 。
