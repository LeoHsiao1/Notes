...
- 对于谁有权生成新区块，以太坊早期采用 PoW 共识算法，后来转向了 PoS 共识算法。主要原理：
  ...
  - 每 32 个 slot 称为一个时期（epoch），有 384 秒。
    ...
    - 当一个 epoch 完成，并获得全网超过 2/3 的验证者投票时，该 epoch 变为合理（justified）状态，又称为 safe 状态。
    - 在 justified epoch 之前的 epoch 处于确定（finalized）状态，表示区块几乎不可能被篡改。
...
- 以太坊计划升级到 v2.0 ，大幅提高网络吞吐量，分为 3 个阶段：
  1. 2020 年 12 月部署一条采用 PoS 的区块链，称为信标链（Beacon Chain）。
  2. 2022 年，将信标链与原链合并
  <!-- 3. 分片链，将网络分散到 64 条链上 -->
