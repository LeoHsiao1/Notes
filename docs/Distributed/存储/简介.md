...
- 部署流程如下，参考文档 <https://documentation.ubuntu.com/server/how-to/networking/install-nfs>
  - 在一个主机上，部署 nfs 服务器。
    ...
    - 编辑 `/etc/exports` 文件，导出本机的目录，允许 nfs 客户端访问：
      ```sh
      /test  *(rw,async,no_subtree_check)
        # * 表示允许所有 IP 的 nfs 客户端访问。可指定 IP 范围来提高安全性，比如 10.1.1.0/24
        # rw 表示允许 nfs 客户端对 exports 目录进行 read、write 操作
        # async 表示写操作不会立即 flush 到磁盘，这样写入速度更快，但 nfs 服务器突然断电就可能丢失数据
        # no_subtree_check 表示不检查 nfs 客户端是否有权访问 exports 目录下的子目录，从而提高速度
        # 默认配置了 root_squash ，会将 nfs 客户端的 root 用户，映射到 nfs 服务器的 nobody 用户
      ```
