...
- Kafka 采用自定的格式封装 message ，然后基于 TCP 协议传输，类似于 HTTP 报文。每条 message 包含以下内容：
  - value
...
- `Log Start Offset`
  - ：partition 中第一条消息的偏移量。刚创建一个 topic 时，该值为 0 。每次 broker 清理消息日志之后，该值会增大一截。
- `Log End Offset (LEO)`
  - ：partition 中最新一条消息的偏移量。
  - producer 向 partition 每写入一条消息，broker 就将 Log End Offset 加 1 。
- `High Watemark (HW)`
  - ：partition 允许被 consumer 看到的最高偏移量。
  - partition 的 leader 新增一条消息时，会更新 LEO 的值，并传给 follower 进行同步，同步完之后才将该消息记作 HW 。
...
- `Consumer Committed Offset`
  - ：某个 consumer 在某个 partition 中最后一次消费的那条消息的偏移量。
  - 它由 Coordinator 记录，可以保证在 rebalance 之后 consumer 不会重复消费。
  - Kafka 能确保一条消息成功生产，但不能确保消息被消费，需要客户端主动更新 Committed Offset 。
...
- `Consumer Lag`
  - ：consumer 在消费某个 partition 时的滞后量，即还有多少条消息未消费。
