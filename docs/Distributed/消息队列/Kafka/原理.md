...
- topic 在存储时会拆分成一个或多个 partition 。
  - 每个 partition 可分别存储消息，独立工作。即使一个 partition 故障，其它 partition 也可用于读写消息。
  - 每个 partition 可存储多个副本。即使一个副本故障，其它副本也可用于读取消息，但不能继续写入消息。
  - 每个 partition 会分别记录一个 offset 变量，表示该 partition 中最新一条消息的序号，从 0 开始递增。
- 例：假设一个名为 test 的 topic ，分成 3 个 partition 。简单的生产、消费流程如下：
  1. 生产者发送 10 条消息到该 topic ，分散存储到 3 个 partition 。此时，3 个 partition 分别存储 4、3、3 条消息，记录的 offset 分别为 3、2、2 。
  2. 消费者获取该 topic 的全部消息，即从 3 个 partition 分别获取消息。
  3. 消费者知道自己目前在 3 个 partition 的消费进度分别为 3、2、2 ，于是记录到 Committed Offset 变量。等之后生产者发送了新消息时，就从 3、2、2 位置处继续消费。
- broker 每收到一条生产者发来的消息时，先看它属于哪个 topic ，然后考虑分配到哪个 partition 中存储。
  - 默认的分区策略为 Round-robin ，即轮询各个 partition 。
  - Kafka 会尽量将同一 topic 的各个 partition 存储到不同的 broker 上，从而分散负载。
  - Kafka 会强制将同一 partition 的各个 replica 存储到不同的 broker 上，从而抵抗单点故障。
  - 客户端只需连接 broker 就能生产、消费消息，不必关心消息实际存储在哪个 broker 。
...
- `Log End Offset (LEO)`
  ...
  - 生产者向 partition 每写入一个消息，broker 就将 Log End Offset 加 1 。
- `High Watemark (HW)`
...
- `Consumer Committed Offset`
  - ：某个 consumer 在某个 partition 中最后一次消费的那个消息的偏移量。
...
- 关于顺序性。
  - 单个 partition 中的所有消息按照 offset 顺序被写入，也按照 offset 顺序被读取。属于先入先出的队列，先生产的消息的 offset 更小，会先被消费。
    - 例如生产者写入两个消息 A、B 到一个 partition 中。等消费者读取消息时，顺序一定也是 A、B 。
  - 如果一个 topic 分成多个 partition ，则这些 partition 之间的消息不能控制顺序。
    - 例如生产者写入两个消息 A、B 到两个 partition 中。等消费者读取消息时，不能确定先读取哪个 partition ，因此顺序可能是 A、B ，也可能是 B、A 。
  - 如果用户希望消费者读取消息的顺序，与生产者写入消息的顺序一致，则可以在 topic 之下只创建一个 partition ，实现全局顺序。
    - 缺点：这样不支持并行消费，大幅降低了消费速度。
    - 也可以实现局部顺序：由生产者的业务代码将消息分成不同 type ，根据 type 的哈希值决定将该消息写入哪个 partition 。从而使得相同 type 的消息，总是写入同一个 partition 。
  - 如果用户希望实现严格的顺序，则还需要给生产者配置 `max.in.flight.requests.per.connection=1` ，限制已发送但未收到 ack 的 batch 同时只有 1 个。
