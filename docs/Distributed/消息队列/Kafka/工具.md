...
- `kafka-consumer-groups.sh` 用于管理消费组。用法：
  ...
  - 当一个 consumer group 长时间未提交新的 offset 时（可能是未运行、未消费），之前的 offset 会一直记录在 __consumer_offsets 中。但 topic 可能已将 log.retention.hours 时长之前的消息删除，因此 topic 中的 earliest offset ，大于 consumer group 之前的 offset ，导致 consumer group 无法从之前的 offset 处继续消费，此时只能手动增加其 offset 。
