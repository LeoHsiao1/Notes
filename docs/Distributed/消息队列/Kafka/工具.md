      ...
      --group <name>    # 指定消费组。可多次使用该参数，也可用 --all-groups 指定所有
        ...
        --topic <name>  # 指定一个 topic 。可多次使用该参数，也可用 --all-topics 指定所有
          ...
          --execute --reset-offsets   # 调整对某个 topic 的 offset
            ...
            --to-latest
  ```
  - 当 consumer group 取消订阅 topic 时，Kafka 不会删除其 offset ，导致该 consumer group 的 Lag 越来越大。用户可用 kafka-consumer-groups.sh 脚本来修改、删除 offset 。
  - 当一个 consumer group 的任一成员在线时，不允许修改、删除其 offset ，避免出错。但可以删除整个 consumer group ，此时也会删除其 offset 。
  - 当一个 consumer group 长时间未提交新的 offset 时（可能是未运行、未消费），之前的 offset 会一直记录在 __consumer_offsets 中。但 topic 可能已将 log.retention.hours 时长之前的消息删除，导致 topic 中的 earliest offset ，大于 consumer group 之前的 offset 。此时 consumer 调用 poll() 会不能消费，报错：`Offset out of range` ，于是自动根据 auto.offset.reset 策略重置 offset ，使得下一次调用 poll() 时能正常消费。
