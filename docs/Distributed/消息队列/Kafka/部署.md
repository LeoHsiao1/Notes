...
3. 启动 broker ：
    ...
    - 检查 kraft 状态：
      ```sh
      bin/kafka-metadata-quorum.sh --bootstrap-server 10.0.0.1:9092 describe --status
      ```

### 从 ZK 迁移到 KRaft

对于采用 Zookeeper 模式的旧 Kafka 集群，迁移到 Kraft 模式的流程：
1. 部署至少一个 KRaft 模式的 controller ，`config/kraft/server.properties` 配置文件示例：
    ...
    - 需要查看旧 kafka 集群的 cluseter.id ，将它配置为 controller 的 KAFKA_KRAFT_CLUSTER_ID ：
      ...
      ```
    - controller 启动成功之后，会等待 broker 连接到它，每秒打印一条日志：`Still waiting for ZK brokers [<int>] to register with KRaft`

2. 重启旧 Kafka 的 broker ，升级到 v3.4.0 版本，并将修改 `config/server.properties` 配置文件，使它同时连接 Zookeeper 和 controller ，内容示例：
    ```sh
    # 保留 Zookeeper 模式的旧配置
    broker.id=1
    ...
    zookeeper.metadata.migration.enable=true
    ```
    - 使用 bitnami/kafka 镜像时，需要声明环境变量 KAFKA_ENABLE_KRAFT=false 。
    - 注意 listeners、advertised.listeners、inter.broker.listener.name 要配置相同的名称。
    - 不能让这个旧 Kafka 节点同时担任 KRaft 模式的 broker 和 controller ，否则启动时不能初始化 log.dirs 目录。
    - 迁移模式下，
      - broker 依然工作在 Zookeeper 模式。
      - KRaft controller 会取代 Zookeeper 模式中原来的 controller 节点，因此有权读写 topic、offset 等元数据，将它们迁移到 KRaft 数据目录。
      - 即使停止迁移模式，Zookeeper 中记录的 controller 节点依然是 KRaft controller ，因此不支持从 KRaft 模式回滚到 Zookeeper 模式。
    - 迁移完成之后， controller 节点会打印一条日志：`Completed migration of metadata from Zookeeper to KRaft`

3. 弃用 `config/server.properties` 中的临时配置，改用纯 KRaft 模式的 `config/kraft/server.properties` 配置文件，重启 broker 。
    - broker 配置文件中的 broker.id 改为 node.id ，编号不变。

4. 弃用 zookeeper.metadata.migration.enable 等临时配置，重启 controller 。
