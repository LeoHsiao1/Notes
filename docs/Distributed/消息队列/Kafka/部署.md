...
# 清理数据日志
# log.cleanup.policy=delete               # LogSegment 的清理策略。默认为 delete ，删除保存时长超过 log.retention.hours 的 LogSegment 。可改为 compact ，这会创建新的 LogSegment ，对于各种取值的 key 只保留最新的一条 message ，删除之前的 message
