...
- 可以让 broker 监听多个地址：
  ```sh
  listeners=PLAINTEXT://0.0.0.0:9092,OUTSIDE://0.0.0.0:9093             # 定义两个 listener 。当客户端通过某个 listener 连接时，就采用相应的 advertised.listeners
  advertised.listeners=PLAINTEXT://10.0.0.1:9092,OUTSIDE://1.1.1.1:9093
  listener.security.protocol.map=PLAINTEXT:PLAINTEXT,OUTSIDE:PLAINTEXT  # advertised.listeners 的解析格式
  inter.broker.listener.name=PLAINTEXT                                  # broker 之间通信时采用的 listener
  ```
...
- Kafka 支持将生产的消息进行压缩。
  ...
  - 优点：
    ...
    - 如果 CPU 足够、网速不足，则采用压缩会减少生产、消费的耗时。否则会增加耗时。
  - 缺点：
    - 占用更多 CPU 。
  - 支持多种压缩格式：
    ```sh
