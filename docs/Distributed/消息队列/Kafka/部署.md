...
- Kafka 原本使用 Zookeeper 维护 broker 系统，从 Kafka v3.4 开始弃用 Zookeeper ，改用 Kafka 内置的 KRaft 协议。
  ...
  - 从 Zookeeper 改用 KRaft 的优点：简化 Kafka 集群的部署，大幅提高 broker 之间的通信效率，提高管理大量 partition 时的性能。

- KRaft 模式下，部署 Kafka 集群的流程：
  1. 创建 `config/kraft/server.properties` 文件，配置示例：
      ```ini
      ...
      controller.listener.names=CONTROLLER    # controller 之间通信时采用的 listener ，不能与 inter.broker.listener.name 采用同一个 listener
  ...
  2. 生成集群 ID ：
      ...
      - 使用 bitnami/kafka 镜像时，只需声明环境变量 KAFKA_KRAFT_CLUSTER_ID ，启动时会自动 format 数据目录。
  3. 启动 broker ：
      ```sh
