...
log.retention.hours=48                    # 限制单个 LogSegment 的保存时长，超过则删除。默认为 7*24h
...
# compression.type=producer               # broker 对消息的压缩格式，比如 gzip、lz4、snappy 。取值为 producer 则采用与生产者相同的压缩格式
...
配置示例：
```ini
bootstrap.servers=10.0.0.1:9092,10.0.0.2:9092   # 要连接的 broker 地址，多个地址之间用逗号分隔。即使只填一个地址，也能通过它发现集群的其它 broker
# client.id=''                  # 客户端的 ID
...
# linger.ms=0                   # 生产者发送每个 batch 时，延迟多久才实际发送。调大该值，有助于让每个 batch 包含更多消息。特别是当新增消息的速度，比发送消息的速度更快时
...
```
- producer 向同一个 partition 生产多个消息时，会打包为一批（batch）。
  - 如果出现的第一个消息就超过配置的 batch.size ，则依然会打包成一个 batch ，但只包含该消息。
  - Kafka 生产消息、消费消息、同步 replica 时，都会将消息打包成 batch 再传输，从而提高传输效率，但会占用更多内存。
    - 每次网络请求可能传输 0~N 个 batch 。

- batch.size 不能大于 producer 的 max.request.size ，否则发出的请求不能包含一个 batch 。
