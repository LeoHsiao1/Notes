...
- v2.8.0
  - 于 2021 年发布。
  - 增加一个试用功能：用内置的 KRaft 协议取代 zookeeper ，大幅提高 broker 之间的通信效率，提高管理大量 partition 时的性能。
- v3.0.0
  - 于 2021 年发布。
...
- 启用 KRaft 模式时，需要改用以下配置：
  ...
  ```
  - zookeeper 模式下，各个 broker 会将元数据写入 zookeeper ，实现分布式一致性。选出一个 broker 担任 controller ，管理整个 Kafka 集群。
  - kraft 模式下，各个 broker 不再连接 zookeeper ，而是由一些（建议 3 个或 5 个） broker 担任 controller ，按 KRaft 协议进行分布式系统的决策。其它 broker 跟随 controller 的决策。
