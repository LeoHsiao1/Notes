...
- 假设程序 A 想发送一条消息（比如发送一个邮件），给程序 B 。普通的解决方案是，直接通信。
  ...
  - 缺点：
    - 两者耦合
      - 直接调用对方的 API ，如果对方的 API 变化，或者对方运行故障，则会导致消息发送失败。
    - 两者同步工作
...
- 另一种解决方案是，观察者模式。
  - 原理：
    ...
    - publisher 提供一套关于订阅（subscribe）的 API 。
      ...
      - 如果不存在订阅机制，则 publisher 会将所有新消息发给 subscriber ，而这些消息不一定对 subscriber 有用。
  - 优点：
    - 支持一对多通信
...
- 发布/订阅模式，是观察者模式的改进版。不让 publisher 与 subscriber 直接通信，而是引入一个 broker 作为中介。
- 架构：
