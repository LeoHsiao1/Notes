...
- 用 `$var` 或 `${var}` 的格式可以读取一个变量的值。
  ...
  - `${var}` 格式的功能更多：
    ...
    支持在读取变量时设置默认值：
    ```sh
    [root@CentOS ~]# echo ${a-0}      # 读取变量 a 的值。如果变量不存在，则返回默认值
    1
    [root@CentOS ~]# echo ${c-0}
    0
    [root@CentOS ~]# echo ${c-b}      # 默认值应该为一个常量，不会当作变量名进行取值
    b
    [root@CentOS ~]# echo ${a:-Null}  # 如果变量不存在或为空，则返回默认值
    Null
    ```
    ```sh
    [root@CentOS ~]# echo ${a+True}   # 如果变量存在，则返回默认值
    True
    [root@CentOS ~]# echo ${a:+True}  # 如果变量存在且不为空，则返回默认值
    True
    ```
    ```sh
    [root@CentOS ~]# echo ${c=3}      # 如果变量不存在，则返回默认值，并给该变量赋值
    3
...
$ set            # 显示当前 shell 的所有变量
     ...
     -e          # exit ，开启 e 模式，如果执行某个命令的返回码为非零，则立即退出当前 shell
...
- set -e 模式下，如果同时执行多个命令，则考虑最后一个命令的返回码。如下：
  ```sh
  echo  && echo1  # 最后执行的是 echo1 ，返回码为 127
...
- 例：
  ```sh
  [root@CentOS ~]# let a=1+2      # 表达式中不需要加空格，比 expr 命令更方便
  [root@CentOS ~]# let a++        # 支持自加、自减运算符
  [root@CentOS ~]# let a+=10
  [root@CentOS ~]# echo $a
  14
  [root@CentOS ~]# let b=a*2
  [root@CentOS ~]# echo $b
  28
  ```
