...
- `/var/spool/cron/` 目录下保存了各个用户用 crontab -e 命令编辑的定时任务。格式如下：
  ...
  */1 * * * *  echo hello &>> /var/log/cron   # 每分钟执行一次，并记录输出内容
  ...
  ```
  - 用 # 声明单行注释。
  - 每行定义一条定时任务，格式为 ` 执行时间 + 待执行的命令 ` 。
  - 执行时间包含五个参数：第几分钟、第几小时、每月的第几日、第几月、周几。
  ...
  - 例：
    ...
    - `* */1 * * *` 表示每小时的每分钟执行一次。
    - `*/1` 表示每 1 个单位时间一次，等价于 `*` 。
  - crontab 在执行时只配置了少量的环境变量，比如 `PATH=/usr/bin:/bin` ，可能需要主动加载 /etc/profile 以配置环境变量。
    - 同理，填写待执行的命令时，应该尽量使用绝对路径，避免找不到文件。
...
- crond 会在 `/var/log/cron` 文件中记录执行的每个任务、执行时间，如下：
  ```sh
