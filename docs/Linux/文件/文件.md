...
- sendfile() 是 Linux 系统提供的一种零拷贝技术。
  - 以普通方式拷贝文件内容时，需要调用 read() 拷贝到进程内存空间，再调用 write() 。
    - 而调用 sendfile() 时，不需要拷贝到进程内存空间，直接由内核完成拷贝。
  - 例如 HTTP 服务器发送文件时，可以将磁盘文件用 sendfile() 拷贝到 Socket 文件，提高发送文件的速度。
...
void *mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset);   // 创建映射，返回其内存指针
    // addr   ：用于创建映射区域的虚拟内存地址。为 Null 则自动选择
    // length ：映射区域的大小，只能是内存页大小的整数倍。因此映射小文件时容易浪费内存，更适合映射大文件。
    // fd     ：要映射的目标文件。创建映射之后，关闭文件描述符 fd 并不会取消映射。
...
- MMAP ：Memory Mapped File ，内存映射文件。
  - 是指在进程的虚拟内存空间创建一块区域，映射到磁盘中某个文件的部分内容，即将虚拟内存地址映射到磁盘地址。
  - 进程可以像访问一块动态分配的内存空间一样，读写该文件。
    - 当进程读取文件数据时，如果内存中没有该数据，则引发缺页异常，从磁盘的映射地址拷贝文件数据到内存。
    - 当进程修改文件数据之后，内核会每隔一定时间自动将脏页 flush 到磁盘。
  - 优点：
    - 在内存中读写文件，速度快很多。
      - 用 read()、write() 读写磁盘文件时，需要经过 Buffer 缓冲。而用 MMAP 时，不需要缓冲。
    - 多个进程可以同时映射同一个文件，类似于共享内存，进行进程间通信。
