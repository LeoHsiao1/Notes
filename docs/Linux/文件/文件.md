...
- 每个进程使用的文件描述符都会记录在 `/proc/<PID>/fd/` 目录下。如下：
  ...
  ```
- 删除一个文件时，如果该文件依然被某个进程打开，则不会释放磁盘空间。如下：
  ```sh
  ...
  ```
  - 此时执行 `du -sh .` 命令，会发现当前目录的体积减少了。但是执行 `df -h` 命令，会发现磁盘的已用空间没有减少。需要重启进程，释放文件描述符。
...
```
- unlink() 的原理：
  - 如果 path 是硬链接、软链接文件，则直接删除。
  - 如果 path 是硬链接，且指向的文件的 inode 引用计数为 1 （即只有这一个硬链接），则考虑是否删除该文件：
    - 如果当前没有进程打开该文件，则从文件系统、磁盘删除该文件，释放 inode 。
    - 如果当前有进程打开该文件，则只是从文件系统删除该文件。
      - 此时用 ls 命令不会显示该文件，但 df 命令显示的 used 磁盘空间未减少。
      - 此时已打开该文件的进程依然可以访问它。等进程关闭文件时（比如进程退出），才自动从磁盘删除该文件，释放 inode 。
  - 如果 path 是 socket 或 device 类型的文件，则从文件系统删除它，但已打开该文件的进程依然可以访问它。
- rm 命令就是调用 unlink() 来删除文件。不过 unlink() 一次只能删除一个文件，且不能删除目录。
