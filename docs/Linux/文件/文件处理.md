...
- 例：
  ```sh
  dd if=/dev/sda1 of=/dev/sda2    # 从 if 所指目录拷贝数据到 of 所指目录
  dd if=/dev/cdrom of=cd.iso      # 拷贝文件并转换成 iso 文件
  ```
- 例：拷贝生成一个指定大小、随机内容的文件，这也能测试磁盘的顺序读写速度
  ```sh
  [root@CentOS ~]# dd if=/dev/urandom of=f1 count=1024 bs=1M
  1024+0 records in
  1024+0 records out
  1073741824 bytes (1.1 GB) copied, 6.89061 s, 156 MB/s
  ```
...
- 命令：
  ...
  - rsync 每拷贝一个文件时，会先暂存到 DEST 目录下一个临时文件，比如 .f1.GRds8g 。等拷贝完才把它重命名为 DEST 文件。
    - 启用 -P 选项时，即使未完全拷贝文件，rsync 就中断，也会将临时文件重命名为 DEST 文件，从而能在下一次拷贝时断点续传。
- 例：
  ```sh
