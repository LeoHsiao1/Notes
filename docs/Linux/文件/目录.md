...
- `/tmp` ：用于临时存储文件。所有用户都可以在这里读、写、创建文件，但非 root 用户只能删除自己的文件。系统会定期清理 /tmp 目录。
  - 例如 CentOS 7 系统中，systemd 每天执行一次定时任务，删除 /tmp 目录下超过 10 天没有被使用（包括 Access、Modify、Change）的文件。可以修改配置文件 `/usr/lib/tmpfiles.d/tmp.conf` ，用 x 排除删除指定的目录及其内容，用 X 排除删除指定的目录但不排除其内容。
