...
- 例：
  ...
  ```
- ls -l 命令查看的是文件体积（即包含的数据大小）， du 命令查看的是文件实际占用的磁盘空间。
  ...
  - 即使一个文件体积很大，实际占用的磁盘空间也可能很小。如下：
    ...
    10485760 f1
    ```
- 稀疏文件（Sparse file）：文件预分配了磁盘空间但没有全部使用。
  - 例如，如果一个文件包含大量空字节数据，则这些数据通常不会被文件系统写入磁盘，导致该文件实际占用的磁盘空间小于文件体积。
  - 例如虚拟机的磁盘在宿主机上通常存储为稀疏文件，可能文件体积有 100G ，但实际只包含 1G 的非空数据。
...
：一种压缩文件格式，后缀名为 .tar.gz ，或简写为 .tgz 。通常可以压缩一大半的文件体积。
- 压缩命令：
  ```sh
