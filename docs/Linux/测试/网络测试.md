...
- 命令：
  ...
  tcpdump
          ...
          -w dumps.pcap       # 将二进制的原始抓取信息保存到一个文件中
  ```
  - 过滤表达式支持使用 and、or、not 运算符。
  - 可以先用 tcpdump 抓包并保存为文件，然后在 Wireshark 软件的 GUI 界面中分析。

- 下例是对一次 HTTP 请求的抓包：
  ```sh
  [root@CentOS ~]# tcpdump -nn tcp and dst port 80
...
：一个测试工具，用于测试本机到公网的网速，采用 Python 开发。
...
：一个 GUI 测试工具，提供了通用的网络测试功能，采用 Java 开发。
