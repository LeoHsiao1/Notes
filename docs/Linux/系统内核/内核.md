...
- 关于 netfilter ：
  ```ini
  net.netfilter.nf_conntrack_max = 65536      # 哈希表最多记录多少个 TCP 连接，默认值与主机 CPU 核数、内存成正比
  ...
  - 例：
    ...
    - 如果 client 主动关闭 TCP 连接，则：
      - client 作为主动关闭方，Socket 会变为 TIME-WAIT 状态，等 2*MSL 时长之后才关闭。在 nf_conntrack 中会保留 TCP 连接的记录，停留在 TIME_WAIT 状态，等 nf_conntrack_tcp_timeout_close_wait 超时之后才删除记录。
