...
- 关于 netfilter ：
  ```ini
  net.netfilter.nf_conntrack_max = 65536      # 哈希表最多记录多少个 TCP 连接，默认值与主机 CPU 核数、内存量成正比
  net.netfilter.nf_conntrack_buckets = 16384  # 哈希表中的 bucket 数量，需要设置为 conntrack_max / bucket_size
  ...
  net.netfilter.nf_conntrack_tcp_timeout_syn_recv       = 60      # 可改为 10 秒
  net.netfilter.nf_conntrack_tcp_timeout_syn_sent       = 120     # 可改为 10 秒
  ...
  net.netfilter.nf_conntrack_tcp_timeout_close_wait     = 60      # 可改为 10 秒
  ```
  - 启用 iptables 防火墙时，所有网络流量会经过 netfilter 处理。可执行 `sysctl -a | grep net.netfilter` 查看相关参数。
  - netfilter 使用一个哈希表 nf_conntrack 记录本机所有网络连接（包括 TCP、UDP、ICMP 协议），跟踪它们的状态信息。
    - 哈希表由大量 bucket 组成，每个 bucket 是一个链表结构，记录 bucket_size 条 TCP 连接。每条记录大概占用 320 bytes 的内核内存。
  ...
  - 如果一个网络连接一直没有传输数据、保持空闲超过 nf_conntrack_xx_timeout_xx 时长，则 nf_conntrack 会删除其记录。
    - 即使网络连接已关闭，也要等超时之后才删除，从而能跟踪网络会话信息，实现状态防火墙或 NAT 。
    - 比如一个 TIME_WAIT 状态的 TCP 连接被关闭， nf_conntrack 会保留它的记录，停留在 TIME_WAIT 状态，等 nf_conntrack_tcp_timeout_close_wait 超时之后才删除记录。
    - 减少 nf_conntrack_xx_timeout_xx 超时时间，能更快地删除空闲连接，节约 nf_conntrack 的容量。
  - 相关命令：
    ```sh
  ...
  - 如果主机被大量客户端并发访问（比如每秒访问成百上千次），则容易性能不足。可采用以下措施：
    - 优化服务器程序的代码，减少网络负载，提高效率。
    - 增加服务器的 CPU、内存、带宽等资源。
    - 调整内核参数：
