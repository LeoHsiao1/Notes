...
- 关于 TCP 通信：
  ...
  # TFO（TCP Fast Open）是一项扩展功能，能将 TCP 三次握手缩短一个 RTT 耗时，但存在安全隐患，因此默认禁用
    # 原理：如果同一对 client、server 多次建立 TCP 连接，则 client 可在本机记录 TFO cookie ，等重新 TCP 握手时，在握手的第一步发送的 SYN 包中，携带 cookie 来证明自己的身份。使得 server 在握手的第二步之后，就可以开始向 client 传输数据
