...
- 参考文档：
  - [Sysctl Explorer](https://sysctl-explorer.net/) 支持查询
...
- 关于网络：
  ...
  net.ipv4.tcp_syncookies       = 1       # 是否启用 SYN Cookies 功能。可以取值如下：
      # 0 ：不启用
      # 1 ：默认值，当 SYN 队列满了时启用
      # 2 ：总是启用
  net.ipv4.tcp_abort_on_overflow= 0       # 当 accept 队列满了时，server 怎么处理新的 ESTABLISHED 连接。可以取值如下：
