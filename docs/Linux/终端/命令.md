...
- 通过重定向 stdin、stdout、stderr 的文件描述符，可以转移终端的输入输出。如下：
  ```sh
  echo hello > stdout.log     # 用 > 将 stdout 重定向到指定文件。
  echo hello >> stdout.log    # 用 >> 是追加式写入
  echo hello 2> stderr.log    # 重定向 stderr
  echo hello &> output.log    # 重定向 stdout 和 stderr
  ...
  - 使用 >> 时，会以 O_APPEND 模式打开目标文件。因此当文件长度被截断时，会自动将写入偏移量设置到文件末尾，避免产生稀疏文件。
