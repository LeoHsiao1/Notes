...
$ history         # 显示所有历史命令
          [n]     # 只显示最近的 n 条
          -c      # 清空当前会话的历史命令，但不影响 HISTFILE
          -r      # 读取 HISTFILE 的内容，作为当前会话的历史命令
          -a      # 将当前会话中新增的历史命令追加写入 HISTFILE
          -w      # 将当前会话的全部历史命令写入 HISTFILE ，覆盖原内容
```
- 用户每次登录之后，shell 会从 HISTFILE 加载历史命令到内存中，并记录当前 shell 会话新增的历史命令。
  - 当用户注销登录时，自动将新增的历史命令保存到 HISTFILE 。
