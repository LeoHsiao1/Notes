...
一般的命令都是从 stdin 文件读取输入，将输出写到 stdout、stderr 文件。

Linux 系统启动后最先打开的文件就是 stdin、stdout、stderr 。
...
echo hello &> stderr.txt    # 重定向 stdout 和 stderr
...
有些命令默认会从 stdin 读取内容，因此不输入目标参数时可以使用管道符传入上一条命令的输出；有些命令默认不会从 stdin 读取内容，因此需要使用管道符加 xargs 命令。如下：
```sh
...
```
- 用户输入的历史命令会保存为一个列表，每次用户注销后会保存到 `~/.bash_history` 文件中，执行 history 命令即可查看。
