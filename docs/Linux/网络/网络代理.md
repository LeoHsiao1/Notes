...
3. 重复上述步骤，在第二台主机上部署一个 keepalived ，配置相同，只是 priority 更小，比如 98 。
...
- 用于让几个不同机房的主机，通过公网组成一个虚拟的私有网络，就像它们位于同一个机房。
  - 例如机房 A 的各个主机使用内网 IP `10.1.0.0/16` ，机房 B 的各个主机使用内网 IP `10.2.0.0/16` ，两个机房可以通过 VPN 软件连通，通过内网相互通信。
- VPN 软件与代理软件类似，但区别在于：
...
- OpenVPN
  - 于 2001 年发布。
  - 一个开源的 VPN 协议。基于 OpenSSL 来建立通信隧道、加密通信。

- L2TP/IPsec
  - 于 1999 年发布。
  - 一个开源的 VPN 协议。源自两个传统的隧道通信协议：思科公司的 L2F 协议、微软公司的 PPTP 协议。
  - 通信流程：
    - 主机 A 生成普通的 IP 数据包，被 L2TP 协议封装为 UDP 数据包，然后封装为 IPsec 数据包，发送给主机 B 。
      - L2TP 协议在建立通信隧道时，通过 IKE（Internet Key Exchange）协议，在两个主机之间交换 IPsec 的密钥。
      - IPsec 协议，可以加密传输 IP 数据包的 payload 。
    - 主机 B 收到 IPsec 数据包，解包为 UDP 数据包，然后解包为普通的 IP 数据包。

- IKEv2/IPsec
  - 于 2005 年发布。
  - IKEv2/IPsec 不使用 L2TP 等隧道协议，而是改进 IPsec 中的 IKE 协议，使得用户直接使用 IPsec 协议实现 VPN 通信。
    - 因此，与 L2TP/IPsec 相比， IKEv2/IPsec 减少了封包、解包的次数，性能更好。

- WireGuard
  - 一个开源的 VPN 协议。
  - 于 2017 年发布。于 2020 年集成到 Linux 内核中。
  - 通信流程：
    - 主机 A 生成普通的 IP 数据包，被 WireGuard 协议封装为 UDP 数据包，发送给主机 B 。
    - 主机 B 收到 UDP 数据包，解包为 IP 数据包。
      - 每两个主机之间的通信，使用一个独立的会话密钥，加密通信。

### Tailscale

- Tailscale 是一个提供 WireGuard 服务的 SaaS 平台，于 2019 年发布。
  - [官网](https://tailscale.com)
  - 工作原理：
    - Tailscale 作为一个 Web 服务器运行，记录了许多 WireGuard 节点的地址。
    - 用户可以在一个主机上运行 Tailscale 客户端，连接到 Tailscale 服务器，从而发现其它 Tailscale 客户端，与它们建立 WireGuard 信道。
      - 每个用户的 Tailscale 客户端，只对当前用户可见，不会被其它用户看见。
  - 某些情况下， Tailscale 客户端所在主机，只能单向访问公网，不能被公网访问。导致两个 Tailscale 客户端之间，不能直接通信。
    - 为了解决该问题， Tailscale 公司自研了 DERP（Detoured Encrypted Routing Protocol）协议。
      - 通过一些 DERP 服务器，在 Tailscale 客户端之间中继通信。
      - 只要两个 Tailscale 客户端都能访问到 DERP 服务器的 HTTPS 端口，那么这两个 Tailscale 客户端，就能通过 DERP 服务器，间接通信。
      - 缺点：网络流量需要被 DERP 服务器转发，增加了延迟。

- Headscale 是一个开源软件，模拟了 Tailscale 服务器的功能。
  - [GitHub页面](https://github.com/juanfont/headscale)
  - 用户可以自己部署一个 Headscale 服务器，然后运行 Tailscale 客户端。
  - 用户也可以自行部署 DERP 服务器。

- 部署示例：
  ```sh
  # 部署 headscale 服务器，参考 https://headscale.net/stable/setup/install/official/
  systemctl status headscale

  # 从 https://tailscale.com/download/ 下载 tailscale 客户端，然后启动
  tailscale up \
    --login-server https://headscale.test.com \
      # 连接到 headscale 服务器。此时会显示一个网址，要求你在 headscale 服务器执行一条命令，批准该 tailscale 客户端的连接
    --advertise-routes=192.168.0.0/16 \
      # 是公布本地的子网，供其它 tailscale 客户端访问
    --accept-routes \
      # 接收其它 tailscale 客户端公布的子网
    --reset
      # 重新配置，忽略之前的配置参数

  # 在 headscale 服务器上执行命令，批准 headscale 客户端
  headscale nodes list
  headscale nodes list-routes
  headscale nodes approve-routes --identifier 1 --routes 192.168.0.0/16

  # 测试网络连通，这会显示通过哪个 derp 中继通信
  tailscale netcheck
  tailscale ping 100.64.0.1   # 这是 tailscale 客户端的虚拟 IP
  tailscale ping 192.168.0.1  # 这是 tailscale 客户端公布的内网 IP
  ```
