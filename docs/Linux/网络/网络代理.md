...
- 有的软件不支持使用代理，有的软件只支持使用 HTTP 等简单的代理协议。因此建议安装一个专用的代理客户端，通过两种方式代理其它软件的流量：
  - 透明代理：
    - 拦截其它软件发出的流量，转发到代理服务器。其它软件不会感知到代理，不需要主动使用代理。
  - 转换代理协议：
    - 通过某种代理协议连接到代理服务器，再在本机监听一个采用 HTTP 等简单代理协议的端口，将该端口收到的流量转发到代理服务器。
    - 此时，具有代理功能的软件，一般都能通过该端口使用代理。但没有代理功能的软件，依然不能使用代理。
...
- 特点：
  ...
  - 支持对全局进程启用透明代理。
    - 原理：通过注入 DLL 库，劫持应用程序的 Socket 流量。
  - 支持灵活的代理规则：将某个程序发向某 IP:Port 的 TCP 包，转发到某个代理服务器。
  - 支持用多个代理服务器组成代理链。
- 用法：
  1. 安装 Proxifier 并启动。
  2. 添加代理服务器。
...
：一个 Unix 的命令行工具，采用 C 语言开发，用作代理客户端。
- [GitHub](https://github.com/rofl0r/proxychains-ng)
- 支持对单个进程启用透明代理。
  - 原理：
    1. 声明环境变量 LD_PRELOAD ，让程序在导入 DLL 库时，优先导入自定义的 libproxychains4.so 库。
    2. 在 libproxychains4.so 库中，重写了关于 Socket 通信的函数，将程序发出的 TCP 流量转发到代理服务器。
  - 不支持代理 Chrome 等浏览器，因为它们为了安全会避免流量被劫持。
- 安装：
  ```sh
  wget https://github.com/rofl0r/proxychains-ng/archive/refs/tags/v4.14.tar.gz
  tar -zxvf proxychains-ng-4.14.tar.gz
...
4. 配置 iptables 规则：
    ```sh
    iptables -t nat -A OUTPUT -p tcp -d 192.168.1.0/24 -j REDIRECT --to-ports 12345   # 编辑 nat 表，将 OUTPUT 链发出的某些流量重定向 REDSOCKS 端口
    iptables -nvL -t nat      # 查看 iptables 规则
    ```
5. 保存 iptables 规则：
    ```sh
    iptables-save > /etc/iptables.conf
    ```
    然后执行 crontab -e ，添加一个任务，在系统重启时恢复 iptables 规则：
    ```sh
    @reboot   iptables-restore < /etc/iptables.conf
    ```
...
2. 登录另一个主机，编辑配置文件 frpc.ini ：
    ...
    local_ip = 10.0.0.1
    local_port = 80
    remote_port = 10080     # 让 frps 监听 remote_port 端口，供用户访问，并将该端口收到的网络包转发到 local_ip:local_port
    # bandwidth_limit = 1MB # 限制带宽，默认无限制
    use_compression = true  # frpc 与 frps 之间通信时，是否压缩。默认为 false
    use_encryption = true   # frpc 与 frps 之间通信时，是否加密。默认为 false
