...
- 例：用 docker-compose 部署一个采用 Golang 开发的 SOCKS 5 服务器
  ```yml
  version: '3'
...
：一个代理客户端，属于 GUI 工具。
- 特点：
  - 可以拦截电脑上所有进程发出的 TCP 流量，转发给某个代理服务器，实现正向代理。
...
：一个反向代理工具，采用 Golang 开发，提供了便于内网穿透的功能。
- [GitHub 页面](https://github.com/fatedier/frp)
- 支持 TCP、UDP、HTTP、HTTPS 等多种代理协议。
- 采用 C/S 架构工作。
  - frps ：服务器，部署在任一主机上，只需能被 frpc、用户访问。
    - frps 所在主机通常拥有一个固定的外网 IP ，供外网用户访问。
    - 用户向 frps 发送网络包时，会先被 frps 转发到 frpc ，再被 frpc 转发到内网服务。
  - frpc ：客户端，部署在任一主机上，只需能访问到 frps、内网服务。
...
1. 登录一台主机，下载 frp 的发行版，编辑配置文件 frps.ini ：
    ...
    或者用 Docker 启动：
    ```sh
    docker run -d -p 7000:7000 -v ./frps.ini:/etc/frp/frps.ini --name frps snowdreamtech/frps
    ```
...
2. 登录另一个主机，编辑配置文件 frpc.ini ：
    ...
    或者用 Docker 启动：
    ```sh
    docker run -d -v ./frpc.ini:/etc/frp/frpc.ini --name frpc snowdreamtech/frpc:0.37.0
    ```
