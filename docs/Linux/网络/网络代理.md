...
- 可安装 ipvsadm 命令来管理 IPVS 。
  ...
  ipvsadm
      ...
      # 反向代理模式
      -g              # --gatewaying ，直接路由，这是默认模式
      -m              # --masquerading ，开启伪装功能，在转发数据包时保留源 IP ，否则会使用本机 IP 作为源 IP 。这属于 NAT 模式
      -i              # --ipip ，TUN 模式
  ```
  - 例：
    ```sh
    ipvsadm -A -t 192.168.0.1:80 -s rr                # 新增一个 IPVS ，VIP 及监听端口为 192.168.0.1:80 ，负载均衡算法为 rr
    ipvsadm -a -t 192.168.0.1:80 -r 172.16.0.1:80 -m  # 在 IPVS 中新增一条规则，反向代理 real-server 172.16.0.1:80 ，反向代理模式为 NAT
    ```
