...
- iptables 默认定义了四个规则表：
  - raw
  - mangle
  - nat ：常用于 DNAT、SNAT 地址转换。
  - filter ：常用于定义流量出入规则，过滤数据包。
    - 每个规则表包含多个规则链。
...
- 常见的几种动作（target）：
  ...
  - DROP ：丢弃数据包，并且不作出回复。这可能导致发送方误以为网络不通。
  - LOG ：记录日志。
...
- 例：给 nat 表添加规则
  ```sh
  iptables -t nat -A OUTPUT -p tcp -d 192.168.1.0/24 -j REDIRECT --to-ports 80                      # 采用 REDIRECT 动作，将发向指定目标 IP 的流量，重定向到本机的 80 端口
  iptables –t nat –A PREROUTING –i eth1 –d 1.1.1.1 –dport 80 –j DNAT --to-destination 10.0.0.1:80   # 采用 SNAT 动作，修改目标 IP、PORT
  iptables –t nat –A POSTROUTING –o eth0 –s 10.0.0.1 –j SNAT --to-source 1.1.1.1                    # 采用 SNAT 动作，修改源 IP
  ```

- 例：记录日志
  ```sh
  iptables -A OUTPUT -s 192.168.21.0/24 -j LOG --log-level info --log-prefix '<iptables log>'
