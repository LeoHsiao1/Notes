...
- iptables 内置了 5 个规则表（rule table），用于让防火墙对数据包执行不同操作：
  - raw
...
- iptables 内置了 5 种规则链，用于在不同时间点处理数据包：
  ...
  ```
  - netfilter 只支持在这些时间点创建 hook 函数，因此内置了这些规则链。
  ...
  - 网口发送数据包时，可能发送到当前网口、本机的其它网口、其它主机。
    ...
    - 如果是转发数据包，则源地址、目的地址都不是当前网口，因此不会执行 OUTPUT 链，而是执行 FORWARD 链。
  - 执行某个链时，如果多个规则表中都存在该名称的链，则按照这些规则表的优先级顺序，依次执行不同表中的同名链。
...
- 例：
  - 本机生成数据包，让网口发送时，流程如下：
    ...
    3. 网口将数据包发出去。
  - 网口从其它网口、其它主机接收数据包时，先执行 PREROUTING 链。然后执行 IP 路由：
    - 如果数据包发向当前网口，则：
      1. 执行 INPUT 链。
...
- Linux 内核每次接收、发送一个数据包时，会按以下流程处理：
  1. 按照 `raw > mangle > nat > filter > security` 的优先级顺序，依次读取各个规则表。
...
- 例：对数据包记录日志
  ...
  - 各字段的含义：
    ```sh
    IN    # 该数据包从哪个网口接收。如果数据包是本机生成的，则该字段为空
    OUT   # 该数据包从哪个网口发出。如果数据包被本机接收，则该字段为空
...
- 缺点：
  - 只能转发其它主机发给本机的数据包，不能转发本机内部发送的数据包。因为 firewall-cmd 的端口转发，相当于 iptables 的 nat 表的 PREROUTING 链，不如 iptables 的功能多。
