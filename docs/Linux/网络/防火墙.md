...
- iptables 会将用户配置的规则传给内核的 netfilter 网络包过滤器处理。
...
- 每类规则称为一个规则链。常见的几类规则：
  - INPUT ：处理被当前网卡接收的数据包。
  - OUTPUT ：处理从当前网卡发出的数据包。
  - PREROUTING ：处理被当前网卡接收的数据包，在 IP 路由之前处理，比如修改目标 IP 。
  - POSTROUTING ：处理从当前网卡发出的数据包，在 IP 路由之后处理，比如修改源 IP 。
  - FORWARD ：处理路由转发的数据包。这些数据包的目标 IP 不是当前网卡，因此需要路由转发。
    - 当前网卡设备收到数据包时，会先后经过 PREROUTING 规则链、IP 路由、INPUT 规则链处理，然后写入 Socket ，供应用程序读取。
    - 应用程序将数据包写入 Socket 时，会先后经过 OUTPUT 规则链、IP 路由、POSTROUTING 规则链处理，然后由网卡设备发送出去。
    - 当前网卡设备收到需要路由转发的数据包时，会先后经过 PREROUTING 规则链、IP 路由、POSTROUTING 规则链处理，然后由网卡设备发送出去。
- 常见的几种动作（target）：
  - ACCEPT
    - ：允许数据包通过。
  - REJECT
