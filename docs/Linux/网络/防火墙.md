...
- 例：给 nat 表添加规则
  ...
  iptables -t nat -A OUTPUT -p tcp -d 192.168.1.0/24 --dport 80 -j DNAT --to-destination 10.0.0.1:80  # 采用 DNAT 动作，修改数据包的目标 IP、PORT
  iptables -t nat -A POSTROUTING -p tcp -s 10.0.0.1 -j SNAT --to-source 1.1.1.1                       # 采用 SNAT 动作，修改数据包的源 IP
  ```
