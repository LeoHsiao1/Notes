...
- 计算机中存在读写速度不同的多种存储设备：CPU 寄存器 > CPU Cache / Buffer >> 内存 >> 磁盘
  - 时钟频率、访问延迟也是这样的顺序。
  - 成本则相反顺序。一般磁盘的 IO 速度最慢，但成本最低，因此相同价格时的容量最大。
  - 除了读写速度的差异，外存通常在断电之后能持久保存数据，而内存通常采用 DRAM 等材质，在断电之后会丢失数据。

- CPU 处理数据的速度，比外存读写数据的速度，快很多倍。让 CPU 同步读写外存时，会浪费时间等待 IO 。因此采用异步读写，通过内存、Cache 中转数据。
  - 读取文件的流程示例：
