...
- CPU（Central Processing Unit，中央处理单元），又称为中央处理器（Central Processor）。是计算机的核心元件，负责读取程序、解析成指令并执行。
...
- CPU 中的每个 Core 同时只能执行一个程序（或者说一个线程），但 CPU 经常会在执行一个程序的过程中，转去执行其它程序。该过程称为上下文切换（Context Switch）。
...
- 现代 CPU 的时钟频率通常为 3~4 GHz ，执行指令的速度存在上限。为了更快地执行指令 ，通常在一个 CPU 芯片中包含多个核心（Core）处理器，从而能并发执行多个指令。简称为多核 CPU 。
  - 通常包含偶数个 Core 。
  - 每个 Core 独立工作，分别包含一份运算器、控制器、寄存器等元件。
