...
2. 安装 NVIDIA 公司发布的 [显卡驱动](https://docs.nvidia.com/cuda/cuda-installation-guide-linux) ，或者等下一步与 cuda 一起安装。
    ...
    - 安装之后，会附带 `nvidia-smi` 命令，它提供了 NVIDIA 的系统管理接口（System Management Interface）。
      - 执行该命令，会显示显卡驱动的当前版本、最高兼容的 CUDA 版本（而不是当前安装的版本）、GPU 上正在运行的进程列表。
    ...
    - ubuntu 操作系统可能自动更新显卡驱动，导致不兼容。建议禁止自动更新：
      ```sh
      # 固定 nvidia 相关的软件版本
    ...
    - 安装之后，可执行以下命令来卸载：
      ...
      lsmod | grep nvidia       # 检查 nvidia 相关的内核模块
      rmmod nvidia_uvm nvidia   # 停止运行模块。如果它们正在被某些进程使用，则不能停止
      ```

3. 安装 NVIDIA 公司发布的 [cuda-toolkit](https://developer.nvidia.com/cuda-toolkit-archive) ，它包括 cuFFT 等加速库、CUDA 开发及调试工具、编译器、运行时。
    - 可以用 apt 等命令进行安装。
    ...
    - 可执行以下命令，查看已安装 CUDA 的版本号：
      ...
      - 同一主机或容器中，可以安装多个版本的 CUDA 工具包，共用同一个 NVIDIA 显卡驱动。
    - 安装之后，可执行以下命令来卸载：
      ```yml
