...
：内存中的一块区域，用于缓冲内存向磁盘读写的数据。
- 优点：
  - 内存将数据累积到一定量之后再读取、写入磁盘，降低磁盘的 IO 频率，减少内存等待磁盘 IO 的耗时。

### Cache

：内存中的一块区域，用于缓存 CPU 经常向内存读写的数据。
...
- 一般的计算机中，几个模块的 IO 速度具有很大差距：CPU >> Cache >> 内存 >> 磁盘 。
  - 使用 Cache 时，可以避免 CPU 直接读写内存、磁盘，减少 CPU 等待 IO 的时间。
  - 内核读写磁盘中的数据时，总是要经过 Buffer ，但只有部分数据会载入 Cache 。
- 目前的 CPU 一般采用多级缓存。
  - 例如划分 L1、L2、L3 三级缓存，CPU 先到 L1 Cache 中读取数据，如果 Miss 了再到 L2 Cache 中读取数据，以此类推。
