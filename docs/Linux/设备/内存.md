...
- MMU 为每个进程都维护了一张页表（Page Table），将进程的虚拟内存空间中要使用的内容，映射到物理内存的一些 page 中。
  ...
  - 如果 CPU 在物理内存中没有找到进程的 page ，就会抛出缺页异常（page fault），调用缺页异常处理程序。可能原因包括：
    - 主缺页异常：数据还没有被 MMU 载入内存，需要等待磁盘 IO 。
    - 次缺页异常：数据被载入内存了但是找不到，或不能访问。
  - 当进程申请动态分配一块内存之后，内核并不会立即分配相应的物理内存。而是等进程首次访问该内存时，CPU 会发现该内存不存在而报出缺页异常，MMU 才分配物理内存。
...
- 通过 `/proc/meminfo` 文件可以获取主机内存的使用情况。
  - 通过 `alloc_pages()` 分配的内存不会被统计到已用内存中。
...
- VIRT ：Virtual Memory ，进程申请分配的虚拟内存。
- RES ：Resident Memory ，进程的常驻内存，即 VIRT 中实际使用的部分。
  - 比如进程申请了 100M 内存，实际只使用了 10M ，则 VIRT 为 100M ，而 RES 为 10M 。
  - RES 包括堆、栈、共享内存，不包括 Swap ，也不包括 page tables、huge page、kernel stack、struct thread_info、struct task_struct 等。
  - 进程占用的非共享内存 = RES - SHR 。
  - 用如下命令可以统计所有进程的 RES 内存之和，但这样会重复累计 SHR 内存，因此比实际占用的内存量偏大。
    ```sh
    ps -eo rss | awk 'NR>1' | awk '{sum+=$1} END {print sum / 1024}'
    ```
    建议用以下命令统计所有进程占用的内存之和：
    ```sh
    grep Pss /proc/[1-9]*/smaps | awk '{sum+=$2}; END {print sum}'
    ```
