...
- VAS 的优点：
  - 隔离不同进程的内存空间。这样每个进程，不能访问其它进程的内存地址，除非是共享内存。
  - 可以将物理内存的分散地址映射到 VAS 的连续地址，从而简化进程的寻址。
  - 延迟分配内存：当进程调用 malloc() 申请分配一块内存时，内核并不会立即分配物理内存 pages ，而是先分配一块虚拟内存 pages 。
...
- MMU 是 Linux 内核中的内存管理单元（Memory Management Unit）。
  - MMU 为每个进程维护了一张页表（Page Table），将该进程的虚拟内存空间中已使用的每个 page ，映射到物理内存的某个 page 。
    - 换句话说，页表负责将进程直接读写的虚拟内存地址，映射到实际存储数据的物理内存地址。
    - 页表会为每个 page 记录一条映射关系。在 32 位主机中，每条记录占用 4 bytes 内存空间。在 64 位主机中，是 8 bytes 。
  - 当进程读写虚拟内存中某个 page 的数据时，内核会到 Page Table 中查找它映射到哪个物理内存 page ，从而读写物理内存中实际存储的数据。
    - 如果没有找到映射的物理内存 page ，则抛出缺页异常（page fault），并执行缺页异常处理程序。又称为缺页中断、页面错误。
...
- 缺页异常分为几种：
  - 主要缺页异常（major page fault）
    ...
    - 原因举例：
      ...
      - 使用 swap 分区时，每次 swap in ，会触发一次 major page fault ，将磁盘 swap 分区中的某些 pages 拷贝回物理内存。
...
- 缺页异常的常见用途：
  - 延迟分配内存：
    - 当进程调用 malloc() 申请分配一块内存时，内核并不会立即分配物理内存 pages ，而是先分配一块虚拟内存 pages ，并返回内存指针给该进程。
    - 等进程首次读写该虚拟内存 pages 时，在 Page Table 中找不到映射的物理内存 pages ，会触发 minor page fault 。此时内核才修改 Page Table ，选取一组空闲的物理内存 pages ，映射到虚拟内存 pages 。如果空闲内存不足，则可能触发 OOM 。
...
- 当 Linux 主机运行一些程序之后，物理内存通常分成几块用途不同的空间：
  - 被内核占用的内存
  - 被各个进程占用的内存
...
- 当主机的空闲内存耗尽时，就不能创建新进程，旧进程也不能申请新内存空间。此时，MMU 可能采取三种措施：
  - 按 LRU 算法回收最近不用的 pages 。
  - 将不常用的内存 pages ，从物理内存移到 swap 分区。
  - 通过 OOM 杀死某些进程，释放其占用的内存 pages 。
...
- 主机运行一段时间之后，各个进程可能多次申请小块内存、释放小块内存。导致物理内存空间中，存在很多小块的空闲内存，由地址不连续的 pages 组成。
  - 即使空闲内存比较零散，但 Linux 可以将进程的虚拟内存空间中一组地址连续的 pages ，通过 MMU 映射到物理内存中一组地址不连续的 pages ，从而方便进程在虚拟内存空间中连续寻址。
    - 例如：进程申请分配一块 100MB 大小的内存空间时，如果物理内存中，每块空闲内存的体积都小于 100MB ，则 Linux 可以将多块空闲内存组合起来，凑成 100MB 的总体积，然后映射到虚拟内存空间。（实际上，等触发缺页异常时，才会映射）
  - 如果空闲内存非常零散，存在一些体积小得无法被使用的空闲内存，则将它们称为内存碎片。

- 内存碎片（memory fragmentation）分为两类：
  - 内部碎片
    - ：已分配给进程的内存碎片。
...
- Slab
  ...
  - Slab 内存空间分为两种：
    - Unreclaim ：不可回收的。
    - Reclaimable ：可回收的。
  - 可执行命令 `cat /proc/slabinfo` 查看 Slab 内存空间的使用情况。也可执行命令 `slabtop -c` 进行分析。
