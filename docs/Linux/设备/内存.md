...
- Linux 中，每个进程都运行在一个独立的虚拟内存空间中，就像系统中只有它一个进程。
  - 注意它属于一个逻辑概念，与 Windows 的虚拟内存不同，后者类似于 Linux 的 Swap 分区。
...
- MMU 为每个进程都维护了一张页表（Page Table），将进程的虚拟内存空间中要使用的内容，映射到物理内存的一些 page 中。
  ...
  - 当进程想访问某个内存空间时，CPU 会先获取其虚拟内存地址，然后到 Page Table 中找到对应的物理内存地址，访问其中的数据。
  - 当进程申请动态分配一块内存之后，并不会立即获得该内存，而是等到它首次访问该内存时，CPU 会发现该内存不存在而报出缺页异常，MMU 才将物理内存分给它。
...
- 例：
  ...
  - used ：已被占用的物理内存。
    ...
    - 已用内存的成分包括：
      ```sh
      process resident memory   # 主要成分
      page tables
      kernel modules            # 可用 lsmod 命令查看
      kernel stack              # 内核会给每个线程分配一个栈区，程序陷入内核态之后才可以访问，默认为 16KB
      ...
      ```
      - 通过 alloc_pages() 分配的内存不会被 `/proc/meminfo` 统计。
