...
- Linux 每次触发 OOM killer 时，会记录内核日志。如下：
  ...
  - 也可执行命令 `journalctl --priority=err | grep -i "Out of memory"` 查看 OOM 日志。
...
- 用户空间的内存分配器（memory allocator）有多种，如下，都提供了 malloc() 等 API 供用户调用。
  - ptmalloc
    ...
    - v1 版本，会为每个进程创建一个内存分配区 arena ，每个线程调用 brk() 或 mmap() 分配内存时，都需要短暂对 arena 加锁，因此容易多线程容易等待锁，增加耗时。
    - v2 版本，会为每个进程创建最多 8 × CPU 核数的 arena ，每个 arena 是一块至少 64MB 的虚拟内存。这样能减少多线程等待锁的耗时，但容易增加内存碎片。
      - 用户可以配置环境变量 `export MALLOC_ARENA_MAX=2` ，限制 arena 数量。
  - jemalloc
    - ：由 FreeBSD libc 库提供。
  - tcmalloc
    - ：由 Google 公司开发。
    - 新建每个线程时，预先申请一块 ThreadCache 内存（初始体积为 64KB ，允许扩容）。这样能快速给线程分配一些 ≤32 KB 的内存来存储 small object ，还能减少内存碎片。
