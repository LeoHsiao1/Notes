...
- 用户空间细分为几块用途不同的区域，从高地址到低地址排列如下：
  ...
  <empty>
  mmap      # 包含一组 VMA 区域，通过 malloc()->mmap() 映射
  <empty>
  ...
  - stack 区域主要被函数使用，调用函数时会自动分配内存来存储局部对象，函数执行完之后会自动销毁局部对象、释放内存。
    - 每次调用一个函数，会将函数的实参拷贝到 stack 中，作为函数的形参（属于函数内部的局部变量）使用。
    - 每次调用一个函数，会将函数调用方的地址记录到 stack 中。这样当程序执行完该函数之后，才知道返回哪处代码继续执行。
  - heap 区域主要被 malloc() 使用，由用户主动决定什么时候申请内存、申请多少内存、什么时候释放内存。

- 理论上说，如果使用同一程序启动多个进程，则 data、BSS、text 区域的体积相同，开始地址相同。
