...
- 动态 RAM（DRAM）
  ...
  - 常用于制作计算机的内存，已演变出多代技术：
    ...
    - DDR2 SDRAM ：读写速度大概为 5 GB/s 。
    - DDR3 SDRAM ：读写速度大概为 10 GB/s 。
    - DDR4 SDRAM ：读写速度大概为 30 GB/s 。
...
- 用途：缓冲从内存写入磁盘的数据。
  ...
  - 缺点：
    - 进程读取磁盘时，所有数据都要先被内核拷贝到 Page Cache ，再拷贝到进程内存空间。写磁盘的流程反之。因此耗时多一些，占用内存多一些（尽管 Cache 会自动释放）。
    - 写数据时，需要保证将脏页同步到磁盘。
...
- 修改缓存中的数据时，需要保证缓存与磁盘的数据一致性，主要有三种策略：
  ...
  - 写回（write back）
    ...
    - 与写透缓存相比，减少了磁盘 IO 次数，效率更高，但实现难度更大。
    - Linux 的 Cache 默认采用该策略。
      - Linux 会自动保证 Cache 与内存的数据一致性，但不会保证 Cache 与磁盘的数据一致性。
      - 比如主机突然宕机，脏页可能来不及同步到磁盘，导致数据丢失。
...
- 计算机的物理内存，大部分被进程消耗，少部分被内核消耗。
  ...
  - 内核占用的内存包括：
    ...
    Cached
    Slab
    Sockets           # 每个 Socket 大概占用 3 KB 内存，对应的 IO 缓冲区最多占用 200 KB
