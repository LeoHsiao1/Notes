...
- 用户空间细分为几块用途不同的区域，从高地址到低地址排列如下：
  ```sh
  stack           # 栈区，用于存放局部变量、函数参数（包括形参、实参）
  <empty>
  mmap segment    # 包含一组 VMA 区域，通过 malloc()->mmap() 映射
  <empty>
  heap            # 堆区，该区域的内存可以被 malloc()->brk() 动态分配
  <offset>
  data segment    # 数据段，用于存放已初始化的全局变量、静态变量
  BSS segment     # 用于存放尚未初始化的全局变量、静态变量
  text segment    # 代码段，用于存放该进程要执行的程序代码
  ```
  - 如果使用同一程序启动多个进程，则 text、BSS、data 区域的体积相同，首地址相同。
  - stack、mmap、heap 三个区域的体积都不固定，可能在进程运行时增长。
    - 因此 Linux 在这三个区域之间，预留了大块未使用的虚拟地址。
    - 而且 stack 区域是从高地址向低地址，倒序增长。
...
- 页表的缺点：
  - 使用页表时，CPU 需要先读取页表，查询到 frame 地址，然后读取该 grame 中的数据。需要两次读取，这拖慢了 CPU 运行进程的速度。
    - 对策：使用 TLB 缓存
...
- malloc() 有两种底层实现方式。
  - 早期的 malloc() ，是从进程的 heap 分配一块内存，给进程使用。
    - 原理：
...
- 常见的几种内存分配器：
  - ptmalloc
    ...
    - 只有一个内存分配区，多线程时需要频繁加锁，导致 CPU 负载增加。
  - jemalloc
    - ：由 FreeBSD libc 库提供。
    - 根据 CPU 核数划分多个内存分配区，从而减少多线程时的加锁。
  - tcmalloc
    - ：由 Google 公司开发。

### 分配策略

- 对于进程而言，它只管调用 malloc() 申请内存。对于操作系统而言，需要考虑内存分配策略，作用于所有进程，从而提高性能。
