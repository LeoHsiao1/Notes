...
- 磁盘设备中默认没有划分磁盘分区，可选创建多个磁盘分区，适合处理复杂任务：
...
```sh
mkswap
      [device]    # 将一个 device 格式化成 swap 分区
```
- 例：
  ```sh
...
- 相关命令：
  ...
  ```
  ```sh
  resize2fs <device> [size]   # 调整文件系统的容量
  ```
  - resize2fs 命令支持处理 ext2、ext3、ext4 文件系统。
    - 文件系统已挂载时，支持在线扩容，但不支持在线缩容。
