...
- 使用步骤：
  ...
  4. 将逻辑卷格式化为文件系统，挂载后供用户读写。
- 逻辑卷的分类：
  - 线性卷（linear volume）：将多个物理卷串联，从实际地址映射到逻辑卷的连续地址。
  - 条状卷（striped volume）：将多个物理卷并联，数据轮流写入各个物理卷。
  - 镜像卷（mirrored volume）：数据只写入一个物理卷，其它物理卷会同步拷贝，作为备份。
  - 快照卷（snapshot volume）：对相同 VG 中的另一个逻辑卷进行快照备份。
    - 采用写时复制（Copy-On-Write），快照卷内先保存指向数据 inode 的硬链接，当数据变化时才拷贝到快照卷。因此不会中断服务，支持增量备份。
