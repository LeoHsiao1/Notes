...
以不挂断（no hang up）的方式执行一条命令，会忽略 SIGNUP 信号，还会忽略 stdin 。默认将 stdout、stderr 保存到 ./nohup.out 文件中。

用法：
```sh
nohup <命令>
nohup <命令> &                 # 将进程移到后台运行，以免阻塞前台
nohup <命令> &> nohup.out &    # 重定向输出
```

例：根据 cmdline 杀死进程，再用 nohup 重启
```sh
CMD="ping localhost"

echo "Restarting process: $CMD"
