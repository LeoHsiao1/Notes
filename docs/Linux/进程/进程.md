...
- 程序终止的几种情况：
  ...
  - 接收到 SIGTERM 等信号，被杀死。
- Unix 系统要求程序结束执行时返回一个整数值，称为退出码（Exit Code）、返回码（Return Code），用于表示程序的执行结果。作用类似于 C 函数的 return 值。
  - 程序退出码的取值范围为 0~255 。通常取值为 0 时表示正常退出，取值为非 0 时表示异常退出。
...
- flags 举例：
  ...
  #define CLONE_PARENT         0x00008000 // 使得新进程的父进程，指向本进程的父进程。如果不启用该标志，则新进程的父进程是本进程
...
- 在 Linux 中启动一个程序，就会创建一个进程，让 CPU 执行该程序的代码。但进程可以调用 execve() ，抛弃当前程序，改为执行另一个程序。
  ...
  - execve() 函数实际上没有返回码，因为当前进程的代码段、堆栈被替换了，没有正在执行的 execve() 函数。一旦执行完新程序的 main() 主函数，当前进程就会退出。
...
void exit(int status);
    // 让当前进程退出，且退出码取值为 status
```
- 如果一个进程包含多个线程，则任一线程调用 exit() ，都会导致整个进程退出。
