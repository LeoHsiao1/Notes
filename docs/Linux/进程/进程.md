...
- 操作系统启动每个程序时，会创建一个进程（Process），来代表这个程序的存在。
  - 进程是程序运行、分配系统资源的最小单位。
  - 当进程被删除时，就代表该程序结束运行。

- 一个进程只能运行一份程序代码。但可以创建任意个其它进程，然后让这些进程执行任意的程序代码。
  - 创建的新进程，默认会担任当前进程的子进程。也可以修改配置，不担任子进程。

- 进程组（Process Group）
  - ：包含一个父进程，和多个子进程。也可以没有子进程，只存在父进程。
...
Linux 系统会给每个进程、线程分配一些标识符（ID），包括：
...
- S ：Sleeping
  ...
  - 例如，HTTP 服务器平时处于 Sleeping 状态，收到 HTTP 请求时才切换到 Running 状态。
- D ：Disk Sleep
...
- Linux 定义了多种信号，常见的如下：
    ...
    - SIGKILL、SIGSTOP 两种信号，不能被进程忽略或捕捉，因此一定会立即执行。
...
- 每个进程用一个 task_struct 结构体记录其元数据，称为进程控制块（Process Control Block ，PCB）、进程描述符。如下：
  ...
  - 关于 task->mm ：
    ...
    - 内核线程没有虚拟内存空间，因此 task->mm 取值为 null 。而 task->active_mm 用于记录最近运行的一个用户态进程的 task->mm ，从而访问其中的数据。
...
- fork() 会将本进程的大部分上下文拷贝给新进程，例如：
  - 拷贝本进程的虚拟内存空间。
    ...
    - 直接拷贝整个虚拟内存空间的开销较大，因此这里采用写时复制方式，当新进程修改某块内存数据时才拷贝。不过依然需要拷贝本进程的 Page Table ，存在少量时间、内存开销。
  - 拷贝本进程打开的文件描述符列表。
...
```
- 在 Linux 中启动一个程序，就会创建一个进程，让 CPU 执行该程序的代码。但进程可以调用 execve() ，抛弃当前程序，改为执行另一个程序。
  - execve() 会新建一个虚拟内存空间给新程序使用，删除旧的虚拟内存空间。因此代码段、数据段、堆栈等内容都不同。
  - execve() 不会影响当前进程的 PID 、打开的文件描述符列表。
  - execve() 函数实际上没有返回码，因为 execve() 所在的旧程序已被抛弃，以后执行的是新程序。一旦执行完新程序的 main() 主函数，当前进程就会退出。
- 调用 fork() 加 execve() ，就可以切换子进程执行的程序。与之相比，调用 clone() 并指定 fn 函数指针，只是改变子进程执行的主函数。
  - 例：在 bash 终端执行 `ls -l` 命令时，是先调用 `fork()` 创建一个 bash 子进程，然后子进程执行 `execve("/usr/bin/ls", ["ls", "-l"], 0x7ffc0e3d0910 /* 29 vars */)` 切换到 ls 程序。
