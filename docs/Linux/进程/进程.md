...
- R ：Running
  - 进程正在运行。或者说，CPU 正在执行该进程的程序代码。

- S ：Sleeping
  - 此时进程处于睡眠状态，不需要运行。等到满足某些条件时（比如等待 1 分钟、等待磁盘 IO 、等待网络 IO），该进程才继续运行。
...
- D ：Disk Sleep
  ...
  - Disk Sleep 属于不可中断的睡眠状态（Uninterruptible Sleep）。此时，进程正在执行某个不可中断的任务（通常是调用系统接口），没空响应自己收到的信号。
  - 例如：进程等待磁盘 IO 时，会进入短暂几毫秒的 D 状态。此时用 kill 命令发送信号给该进程，并不能杀死它。甚至 kill -9 也不能杀死。
