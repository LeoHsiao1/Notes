...
- 一个主机加入一个网络时，必须绑定一个 IP 地址，才能通过 IP 协议与该网络的其它主机相互通信。
  - 一个主机可以拥有同一个网络的多个 IP 地址。也可以同时加入多个网络，拥有不同网络的多个 IP 地址。
  - 私有 IP 地址：接入私有网络（比如一个局域网）的每台主机，需要绑定一个私有 IP 地址，在该网络中唯一。
  - 公有 IP 地址：接入因特网的每台主机，需要绑定一个公有 IP 地址，在全球唯一。
- 两个主机之间通过 IP 协议传输数据时，不会建立连接。IP 协议只提供尽力而为的数据传输服务：只管把数据发送出去，不进行差错控制。因此是不可靠的通信协议。
