...
- 快速重传（Fast retransmit）
  ...
  - 假设发送方发送 5 个数据包，分别携带 100 bytes 的 payload ，Seq number 分别为 0、100、200、300、400 ，其中第二个包传输失败。则流程如下：
...
- 选择性确认（Selective Acknowledgements，SACK）
  ...
  - 假设发送方发送 5 个数据包，分别携带 100 bytes 的 payload ，而接收方只收到第 0-99、200-499 bytes 的数据。
