...
- 特点：
  ...
  - 传输可靠：
    - 可实现顺序控制。发送的每个数据包都带有序号，接收方可按顺序处理。
    - 可实现差错控制。
    - 可实现流量控制。
    - 可实现拥塞控制。
...
- 确认
  ...
  - 使用序列号，发送方可同时发送多个数据包，不必控制发送顺序，反正接收方能根据序列号对这些数据包排序。
...
- 超时重传
  ...
  - RTO 应该略大于 RTT 。例如 Linux 会根据网络的平均 RTT ，动态设置 RTO 。
...
<!-- - 慢开始：刚建立 TCP 连接时，发送方给 cwnd 设置一个较小的值。之后每经过一次 rtt 往返通信，就将 cwnd 加 1 。 -->
...
- 特点：
  ...
  - 传输不可靠。
- 对比 UDP 与 TCP ：
  - UDP 没有 TCP 的序列号、差错控制等功能，只是简单地发送数据包。
    - 使用 UDP 时，如果需要序列号、差错控制等功能，可在应用层实现。
