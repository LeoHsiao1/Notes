...
- 快速重传（Fast retransmit）
  ...
  - 假设发送方发送 5 个数据包，分别携带 1000 bytes 的 payload ，Seq number 分别为 0、1000、2000、3000、4000 ，其中第二个包传输失败。则流程如下：
    1. 接收方收到 Seq number = 0 的包，于是回复一个 Ack number = 1000 的包。
    2. 接收方未收到 Seq number = 1000 的包，却收到 Seq number = 2000 的包，发现它序列号不符合 Ack number 预期，因此不接受该包，并回复一个 Ack number = 1000 的包。
    3. 接收方收到 Seq number = 3000 和 Seq number = 4000 的包，同理，分别回复一个 Ack number = 1000 的包。
    4. 发送方收到 3 个 Ack number = 1000 的包，判断第二个包传输失败，于是从 Seq number = 1000 处重新发送 payload 。
...
- 选择性确认（Selective Acknowledgements，SACK）
  - ：TCP 通信双方都开启 SACK 功能时，接收方可在 TCP headers 中添加 SACK 字段，声明自己已收到哪些范围的数据，使得发送方可判断哪些数据传输失败，并重传这些数据。
  - 假设发送方发送 5 个数据包，分别携带 1000 bytes 的 payload 。而接收方收到了第 0-1000、2000-5000 bytes 的数据，未收到第 1000-2000 bytes 的数据。
    - 如果未开启 SACK 功能，则发送方需要从 Seq number = 1000 处重新发送，重传第 1000-5000 bytes 的数据，加剧网络拥塞。
    - 如果开启 SACK 功能，则接收方可声明自己已收到第 0-1000、2000-5000 bytes 的数据，使得发送方可判断接收方缺少第 1000-2000 bytes 的数据，并重传这些数据。
...
- 发送窗口
  ...
  - 发送方会自动检查发送窗口：如果发送窗口没满，则可以同时发送多个数据包，等待 ACK 确认。如果发送窗口满了，则暂时停止发送新的数据包。
  - 发送窗口不能超过接收窗口，否则发送的数据包不能放入接收缓冲区，导致丢包。

- 滑动窗口
  - ：接收方将接收窗口的大小告诉发送方，发送方据此配置发送窗口的大小。
