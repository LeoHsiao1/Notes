...
- Java 代码需要先编译才能运行，其流程如下：
  1. 将 Java 源代码编译成字节码，存储在类文件中。
  2. 由 JVM 读取字节码，转换成当前操作系统的机器代码并执行。

- 类文件 ：后缀名为 .class ，用于存储 Java 源文件编译生成的字节码，因此又称为字节码文件。
...
- JVM ：Java 虚拟机（Java Virtual Machine），用于运行 Java 的类文件。
  - Java 语言在不同操作系统上分别实现了 JVM ，使得同一个类文件可以被不同平台的 JVM 直接运行，而不必考虑平台的底层差异，不必修改代码或重新编译。
  - C 代码编译时是直接生成当前操作系统的机器代码，而 Java 代码编译时是生成字节码，以便移植到不同平台的 JVM 上运行。
...
4. 用 java 命令运行类文件（实际上是交给 JVM 运行）：
    ...
    Hello world
    [root@Centos ~]# java Hello.class     # 运行类文件时不需要输入后缀名 .class ，否则会被当做类名的一部分
...
- 构建 Java 项目时，通常将编译生成的类文件、配置文件、资源文件等文件按 ZIP 格式打包，做成后缀名为 .jar 的压缩包。
  - jar 包一般用于存储可直接运行的 Java 程序，或者是 Java 的类库。
  - 构建 Java Web 项目时，通常打包成后缀名为 .war 的压缩包。
- 制品（artifact）：泛指 Java 项目的构建产物。
- 构件：一个独立的功能模块，可在项目中调用其接口。可以保存为源代码、可执行文件等形式。
