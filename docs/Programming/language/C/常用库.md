...
- stdio.h 提供了一些读写文件的函数。

- 调用 fopen() 函数可以打开磁盘中一个文件。
  - 该函数会返回一个结构体 FILE ，代表一个文件流，用户可以从中读、写数据。
  - 打开文件时，有多种模式：
    ```sh
    ...
    a+    # a+r 模式。如果文件不存在则报错
    ```
    - 以上模式，都会将文件中每个字节的数据，当作 char 类型的值处理，称为字符类型的数据流，简称为字符流。
...
- 例：读取文件
  ...
  // 使用 r 模式打开文件时，指针最初指向文件中的第 0 个字节
    // 每读取一个 char 字符，指针就向后移动 1 个字节。移动到文件末尾时，会读取到文件结束符 EOF （宏定义为 -1 ）
  int c;
  while (c != EOF){
...
- 例：使用 fprintf() 和 fscanf() 读写字符流
  ...
  int x = 1, y = 2;
  fprintf(f, "%d%d", x, y);
  fclose(f);                // 关闭文件，再以读取模式打开文件，从而让文件指针重新指向第 0 个字节
...
- 以下函数，用于读写二进制流
  ...
  struct Horse array[COUNT] = {{'A', 1}, {'B', 2}, {'C', 3}};
  ...
  if (fwrite(array, sizeof(struct Horse), COUNT, f) != COUNT)
  {
  ...
  if (fread(array, sizeof(struct Horse), COUNT, f) != COUNT)
  {
