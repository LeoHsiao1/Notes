...
- JRE
  - ：Java 运行环境工具包 (Java Runtime Environment)，只提供了运行 Java 程序所需的 JVM 、类库，因此体积比 JDK 小。
- JVM
  - ：Java 虚拟机（Java Virtual Machine），是一个负责运行 calss 文件的程序。
  - 用 java 命令运行 Java 程序时，原理是启动一个 JVM 进程，读取 class 文件中的字节码，编译成当前操作系统的机器代码，然后执行。
  - JVM 程序通常开发了多个版本，用于适配不同机器平台、操作系统。不同平台的 JVM 向上提供相同的接口，用法相同，使得同一个 class 文件 可以被不同平台的 JVM 运行，而不必考虑平台的底层差异，不必修改代码。
  - C 代码编译是直接生成当前操作系统的机器代码.而 Java 代码编译是生成字节码，以便移植到不器平台的 JVM 上运行。
  - 实现 JVM 功能的软件有多个，最常用的一款是 HotSpot 。
    - 1999 年，Sun 公司发布了 HotSpot ，后来成为了 OpenJDK 的一部分。
    - HotSpot 采用 C++ 语言开发。
...
java
    ...
    # 关于 G1GC
    -XX:+UseG1GC
    -XX:G1HeapRegionSize=1M       # region 大小，取值范围为 1M~32M ，需要是 2 的倍数。默认会根据 InitialHeapSize 自动决定 G1HeapRegionSize ，使得至少创建 2048 个 region
    # region 体积越大，则清理耗时越长
    -XX:MaxGCPauseMillis=200      # 每次 GC 预期 STW 最多停顿多少毫秒，默认为 200 。该值不应该配置得过小，否则会增加 GC 次数，而且不保证绝对低于该时长
