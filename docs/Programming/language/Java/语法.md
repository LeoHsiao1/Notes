...
- JVM 中的线程，存在多种状态：
  - new ：已经调用 new Thread() 创建线程，但尚未启动线程。
  - runnable ：已经调用 thread.start() 启动线程，但尚未被 CPU 执行代码。
  - running ：线程的 run() 代码，正在被 CPU 执行。
  - waiting ：线程正在等待，没有被 CPU 执行。比如调用了 wait() 方法。
  - timed-waiting ：线程正在阻塞等待，并且限制了最长等待时间。比如调用了 sleep() 方法。
  - blocked ：线程正在阻塞等待某个锁的释放。
  - terminated ：线程已终止。
