...
java
      ...
      -XX:NewRatio=2                # old 区域容量是 young 的多少倍，默认为 2 。与 -Xmn 相比，该配置能动态调整
      ...
      -XX:+HeapDumpOnOutOfMemoryError       # 当堆内存耗尽时，生成堆的内存快照，保存到磁盘
      -XX:HeapDumpPath=java_pid$PID.hprof   # 快照文件的保存路径，支持相对路径。它是二进制文件，需要用专用工具分析

      # 关于 GC
      -XX:+UseConcMarkSweepGC       # 选择 GC 算法。比如 UseSerialGC、UseParallelGC、UseG1GC
      -XX:MaxGCPauseMillis=100      # 每次 GC 预期最多停顿多少毫秒，默认不限制。该值不应该设置得过小，否则会增加 GC 次数，而且不保证低于该耗时
      -XX:GCTimeRatio=50            # 取值范围为 1~99 ，默认值为 99 。假设 JVM 累计运行时长为 T ，则 GC 累计耗时预期小于 T / (1+GCTimeRatio)

      # 关于 GC 日志
      -XX:+PrintGCDetails           # 打印 GC 详细日志。默认不打印
      -XX:+PrintGCDateStamps        # 每行 GC 日志的开头加上日期时间戳
      -Xloggc:gc.log                # 将 GC 日志保存到文件中。默认打印到 stdout
      -XX:+UseGCLogFileRotation     # 自动切割日志文件
...
- 建议监控 Java 进程平时实际占用的内存，据此设置内存参数。
  - 建议设置的 -Xms 等于 -Xmx ，让 JVM 启动时就一次向操作系统申请这么多内存。因为频繁申请内存的开销较大。
  - -Xmx 不能设置得太小。如果内存紧张，频繁 GC ，则会导致 CPU 负载大幅增加，甚至进程卡死、端口无响应。
  - JVM 运行时，占用的堆内存可能低于 -Xms 。
