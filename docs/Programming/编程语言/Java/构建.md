...
java
      ...
      -XX:SurvivorRatio=8           # young 区域中，eden space 容量是 survivor space 的多少倍，默认为 8 。默认有 2 个 survivor space ，因此每个的容量为 NewSize/(8+2)
      -Xss1M                        # 每个线程的堆栈大小，默认为 1M
      -XX:MaxDirectMemorySize=??    # Direct Memory 的最大容量，默认等于 -Xmx 。虽然容量大，但一般只使用几十 MB
      -XX:MetaspaceSize=20.8M       # Metaspace 的初始容量，默认为 20.8M 。超过该容量之后，每次扩容 Metaspace 都会触发 Full GC ，因此建议取值等于 MaxMetaspaceSize
      ...
      -XX:InitialRAMPercentage=1.5625     # 如果用户未配置 -Xms ，则将 Cgroup memory 限额的该百分比赋值给 -Xms （这里的百分比必须包含小数点）
      -XX:MaxRAMPercentage=25.0           # 如果用户未配置 -Xmx ，将将 Cgroup memory 限额的该百分比赋值给 -Xmx
      ...
      # 其它配置
      -XX:+OmitStackTraceInFastThrow  # 重复打印一个异常时，省略堆栈信息。默认启用了该功能
```
