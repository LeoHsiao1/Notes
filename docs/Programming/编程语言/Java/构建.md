...
java
      ...
      -Xms 500m                     # -XX:InitialHeapSize ，堆内存的初始容量，默认为主机 RAM 的 1/64 。建议设置成与 -Xmx 相等，否则运行时可能频繁地调整大小，开销较大
      -Xmx 1g                       # -XX:MaxHeapSize ，堆内存的最大容量，默认为主机 RAM 的 1/4 。建议设置得足够大，避免因为内存不足而频繁 GC
