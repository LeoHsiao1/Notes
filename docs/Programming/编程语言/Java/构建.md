...
java
      ...
      -Xms 500m                     # 堆内存的初始容量，默认为主机 RAM 的 1/64 。建议设置成与 -Xmx 相等，否则运行时可能频繁地调整大小，开销较大
      -Xmx 1g                       # 堆内存的最大容量，默认为主机 RAM 的 1/4 。建议设置得足够大，避免因为内存不足而频繁 GC
      -Xmn 300m                     # 堆内存中 young 区域的大小，推荐为整个堆的 1/3
      -Xss 1m                       # 每个线程的堆栈大小，默认为 1M
...
```
- JVM 实际占用的内存可能比 -Xmx 多 1G ，此时说明内存严重不足，会频繁 GC ，导致 CPU 负载大幅增加，甚至进程卡死、端口无响应。
