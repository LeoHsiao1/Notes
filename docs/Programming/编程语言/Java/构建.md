...
- JVM ：Java 虚拟机（Java Virtual Machine），是一个能运行 Java 类文件的程序。
  - 用 java 命令运行 Java 程序时，原理是启动一个 JVM 进程，解释执行 Java 类文件。
...
java
      ...
      -XX:+HeapDumpOnOutOfMemoryError       # 发生 OutOfMemoryError 异常时，自动生成堆快照文件，保存到磁盘
      -XX:HeapDumpPath=java_pid$PID.hprof   # 堆快照文件的保存路径，支持相对路径
      -XX:ExitOnOutOfMemoryError            # 发生 OutOfMemoryError 异常时，终止 Java 进程
      -XX:OnOutOfMemoryError="kill -9 %p"   # 发生 OutOfMemoryError 异常时，执行 shell 命令

      # Java 8u191 新增的配置参数
      -XX:UseContainerSupport             # 该选项默认启用。当 JVM 在容器内运行时，自动根据 Cgroup 的 cpu、memory 限额，设置 -XX:ActiveProcessorCount、-Xmx 的默认值
      -XX:ActiveProcessorCount=<int>      # 设置 JVM 可使用的 CPU 核数
      -XX:InitialRAMPercentage=1.5625     # 将 Cgroup memory 限额的该百分比，设置为 -Xms 的默认值
      -XX:MaxRAMPercentage=25.0           # 将 Cgroup memory 限额的该百分比（要包含小数点），设置为 -Xmx 的默认值
...
- 当 Java 进程申请内存时，如果堆内存的空闲内存不足，则会自动进行 GC 。
  ...
  - 如果 GC 之后依然内存不足，则会抛出 OutOfMemoryError 异常。
    - 建议启用 -XX:+HeapDumpOnOutOfMemoryError 参数，让 JVM 在抛出 OutOfMemoryError 异常时自动生成堆快照文件，方便排查问题。
    - 堆快照（heapdump）是一种二进制文件，记录了所有 Java 对象的内存大小等信息，可用 MAT 等工具分析。
- 建议监控 Java 进程平时实际占用的内存大小，据此设置内存参数 -Xms 和 -Xmx 。
  - JVM 运行时占用的堆内存可能低于 -Xms ，或高于 -Xmx 。
  - 建议设置的 -Xms 等于 -Xmx ，让 JVM 启动时就一次向操作系统申请这么多内存，因为频繁申请内存的开销较大。
  - JVM GC 时，只会销毁垃圾对象，不会将空闲内存释放给操作系统。
    ...
    - 采用 Serial GC 时，可以调整以下配置参数，强制让 JVM 释放空闲内存，但会增加 CPU 开销。
      ...
      ```
  - 在 k8s Pod 中部署 Java 进程时，设置的 limits.memory 应该比 Java -Xmx 的值大一些。因为 Java 进程占用的总内存包括 Heap、Direct Memory、Metaspace 等，可能超过 -Xmx ，如果再超过 limits.memory ，就会导致 Pod 被 OOM 杀死，然后自动重启。
    - 例如，旧的启动命令 `java -Xms1G -Xmx1G  -jar xx.jar` 可改为 `java -XX:InitialRAMPercentage=80.0 -XX:MaxRAMPercentage=80.0 -jar xx.jar`
    - 可在 Pod 中执行 `java -XX:MaxRAMPercentage=80.0 -XshowSettings:vm -version` ，查看自动配置的 Heap Size 。
