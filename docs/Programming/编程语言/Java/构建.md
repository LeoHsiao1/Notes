...
java
      ...
      -Xms500m                              # 设置 Heap 内存的初始大小。建议设置成与 -Xmx 相同大小，避免因为内存不足而频繁 GC 、多次申请分配内存，反正最终会到达 -Xmx 限制
      -Xmx1g                                # 设置 Heap 内存的最大大小。建议设置得足够大，避免因为内存不足而频繁 GC ，导致 CPU 使用率偏高
      -Xss128k                              # 设置每个线程的堆栈大小
...
- 可以声明环境变量 `MAVEN_OPTS="-Xms256m -Xmx512m"` ，限制 mvn 运行时的内存。
- 例：创建一个 webapp 类型的项目
  ```sh
...
- Maven 仓库用于存储 Artifact 的文件，分为两种：
  - 远程仓库
    ...
    - Maven 社区维护了一个远程仓库 `https://repo.maven.apache.org/maven2` ，称为中央仓库（central）。
...
<settings xmlns="http://maven.apache.org/SETTINGS/1.0.0"
  ...
  <!-- 配置远程仓库的访问账号。如果没配置成功，下载依赖时就会报错：Could not transfer artifact: Not authorized -->
  <servers>
    <server>
        <id>releases</id>
