...
```py
__repr__(self)  # 执行 repr(obj) 时，会自动调用该方法
__str__(self)   # 执行 str(obj) 时，会自动调用该方法
```
- 这两个方法的用途不同，返回值不一定相同。
...
```py
__add__(self, other)      # 左加。当对象出现在加号 + 的左边时，会自动调用该方法
__radd__(self, other)     # 右加。当对象出现在加号 + 的右边时，会自动调用该方法

__sub__(self, other)      # 左减
__rsub__(self, other)     # 右减

__mul__(self, other)      # 左乘
__rmul__(self, other)     # 右乘

__truediv__(self, other)  # 左除
__rtruediv__(self, other) # 右除
```
- 例：
  ```py
  >>> class Test:
