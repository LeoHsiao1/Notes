...
- 例：用描述器实现一个装饰器，在赋值时进行类型检查
  ...
  ...             raise TypeError('Expected the parameter: min_value')
  ...         if 'max_value' not in kwargs:
  ...             raise TypeError('Expected the parameter: max_value')
  ...         if not kwargs['min_value'] <= kwargs['max_value']:
  ...             raise ValueError('Expected two parameters: min_value <= max_value')
