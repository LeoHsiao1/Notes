...
```
- 这两个方法的用途不同，返回值不一定相同。
  - `__repr__()` 用于将对象用一个 str 表示，供用户查看这个对象的大致内容。
...
- 内置函数：
  ...
  - 例：
    ...
    'e'
    ```
  - 一般通过 for 语句进行迭代，也可通过 next() 函数进行迭代：
    ```py
    try:
        while True:
            print(next(a))
...
- 如何创建一个可迭代对象？
  - 最低要求是，实现 `__iter__()` 方法。此时可以被 for 语句遍历，不能被 next() 遍历。
  - 进一步地，实现 `__iter__()` 和 `__next__()` 两个方法。此时可以被 for、next() 两种方式遍历。
  - 进一步地，用 yield 进行迭代，更简单，不需要定义 `__iter__()` 和 `__next__()` 两个方法。
