...
```py
def __add__(self, other)      # 左加。当对象出现在加号 + 的左边时，会自动调用该方法
def __radd__(self, other)     # 右加。当对象出现在加号 + 的右边时，会自动调用该方法
...
def __sizeof__(self)    # 用于计算对象占用的内存大小，单位为 bytes 。例如执行 'hello'.__sizeof__()
...
- 描述器的用途，与 `@property` 类似。
  - 不过，描述器只能以类变量的形式创建，然后以实例变量的形式访问。

- 如果一个对象，实现了以下三个方法之一，则属于描述器（descriptor）。
  ```py
  def __get__(self, obj, obj_type)  # 读取该对象的值时，会自动调用该方法
  def __set__(self, obj, value)     # 赋值给该对象时，会自动调用该方法
  def __delete__(self, obj)         # 删除该对象时，会自动调用该方法
  ```
...
- 例：用描述器实现一个装饰器，用于将某个实例方法转换成实例变量
  ...
  ...             return self
  ...         else:
  ...             value = self.method(obj)  # 调用一次实例方法，得到它的返回值
  ...             setattr(obj, self.method.__name__, value) # 给 obj 添加一个实例变量，名称与 method 相同，因此覆盖原本的 method
  ...             return value
  ...
  >>> import random
  >>> class Test:
