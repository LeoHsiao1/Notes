...
open() 是 Python 的内置函数，用于打开磁盘中的一个文件，返回一个文件对象。函数定义如下：
```py
open(
...
- 操作系统中，进程每打开一个文件，会自动分配一个 int 型的文件描述符，相当于文件编号。
  - 在 Windows 系统中，如果一个文件已经被某个进程打开了，就不允许该文件被其它进程修改，直到前一个进程释放文件描述符。
