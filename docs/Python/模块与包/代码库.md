...
Python 库的的发行包主要有两种格式：
- 源代码包：采用 tar、zip 等压缩格式。需要用户解压后，使用其中的 setup.py 文件进行安装。
  - 对于纯 Python 库，源代码包可以直接安装使用。对于非纯 Python 库，可能需要编译之后才能安装、使用。
- wheel 包：采用 ZIP 压缩格式，扩展名为 .whl ，包含预编译的文件，因此用户可以直接安装、使用。
  - 一个非纯 Python 库可能要对于不同平台分别编译 wheel 包，按以下格式命名：
    ```sh
    ...
    # platform tag  ：支持的平台，取值为 any 则不限制
    ```
    - 包名中的各个字段只能包含字母、数字，其它字符都要转换成下划线 `_` 。
    - pypi.com 拒绝上传平台标签为 linux 的 wheel 包，而建议改成 manylinux ，强调该 wheel 包兼容不同的 Linux 发行版。
      - 比如 manylinux2010 是指大概于 2010 年之后发布的 Linux 发行版，支持 x86_64 和 i686 的 CPU 架构。
  - wheel 包名示例：
    ```sh
    Django-3.1.5-py3-none-any.whl         # Django 库的 3.1.5 版本，只支持 Python3 解释器，支持所有平台
    numpy-1.19.5-cp36-cp36m-manylinux1_x86_64.whl
    PyQt6-6.0.0-6.0.0-cp36.cp37.cp38.cp39-abi3-macosx_10_14_intel.whl
    ```

通常通过 setup.py 自动生成源代码包或 wheel 包，步骤如下：
1. 先编写一个 setup.py 文件，配置该 Python 库：
    ...
    setuptools.setup(
        ...
        packages=['pyexiv2','pyexiv2/tests'],                                             # 指定 Python 包目录（不会包括子目录）
        # packages=setuptools.find_packages(),                                            # 自动发现包目录
        package_data={'': ['*.py', '*.md', '*.pyd']},                                     # 将包目录下的哪些文件打包
...
2. 然后执行以下命令进行打包：
    ...
    # twine upload dist/*                             # 将 dist 目录下的 Python 包上传到 pypi.org
    ```
