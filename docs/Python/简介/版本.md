...
- 增加类型注释的语法。
  - 支持在函数中声明形参、返回值的类型。
    ...
    ```
  - 支持给变量赋值时，声明变量的类型，作为注释：
    ```py
    >>> x: int = 1
    >>> x
    1
    ```
  - 如果将类型赋值给变量，则该变量称为类型别名（Type Alias）。
    ```py
    >>> T = int
    >>> x : T = 1
    ```
...
- 支持在 f 字符串中用 ` 变量= ` 的形式打印变量的值，这在调试时很方便。如下：
  ```py
  >>> x = 1
  >>> print(f'{x=}')
  x=1
