...
- 于 2025 年发布：<https://www.python.org/downloads/release/python-3140/>

- 在解释器的交互式 shell 中，显示彩色的字符，进行语法高亮。

- 自由线程成为一个正式功能。
  - 默认禁用，需要用户主动开启。
  - 开启该功能时，每个线程可能降低 5~10% 的性能。

- 允许在一个 Python 进程内运行多个解释器。
  - 以前，如果用户启动多个 Python 进程，则每个进程运行一个独立的解释器，分别存储一份程序代码、变量。
  - 现在，用户可以调用 concurrent.interpreters 模块，在一个 Python 进程中，运行多个解释器。
    - 同一进程中的多个解释器，像多个线程，可以并行运行。但它们不共享 GIL 等资源，接近进程级别的隔离程度。
    - 不过，绝大部分 pypi 包都不支持多解释器，也许等几年之后，该功能才会推广使用。

- 增加语法： t-string 。它与 f-string 的语法相似，但返回值不是一个 str ，而是一个模板。
  - 例：
    ```py
    >>> target = 'World'
    >>> template = t'Hello {target}!'
    >>> template
    Template(strings=('Hello ', '!'), interpolations=(Interpolation('World', 'target', None, ''),))
    >>> list(template)
    ['Hello ', Interpolation('World', 'target', None, ''), '!']
    ```
  - t-string 有什么用途？
    - f-string 会直接将变量的值，插入字符串中。
    - 而使用 t-string ，会分别存储变量的值、原字符串，方便用户进行额外处理，比如将变量的值转换为大写字母、防止被注入攻击。
  - 例：编写一个函数来处理 t-string 模板
    ```py
    >>> from string.templatelib import Template, Interpolation
    >>> def upper_var(template: Template) -> str:
    ...     items: list[str] = []
    ...     for item in template:
    ...         # 获取 t-string 中变量的值，转换为大写字母
    ...         if isinstance(item, Interpolation):
    ...             items.append(str(item.value).upper())
    ...         # 获取 t-string 中原字符串，转换为小写字母
    ...         else:
...
- 改进语法：添加类型注释时，不会立即检查该类型是否存在。
  - 旧语法：
    ```py
    >>> x : int
    >>> x : Myint
    NameError: name 'Myint' is not defined. Did you mean: 'print'?
    ```
  - 新语法：
    ```py
    >>> x: Myint
...
- 改进语法： except 和 except* 表达式中，如果捕捉不止一种异常，并且没使用 as 关键字，则允许省略括号。
  - 旧的语法：
    ```py
    try:
        ...
