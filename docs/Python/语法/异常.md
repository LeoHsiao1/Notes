  ...
  - 常见的一些异常：
    ...
    - BaseException         # 这是所有异常类的基类
      ...
      - Exception           # 普通的异常
        ...
        - OSError           # 表示与操作系统相关的异常，例如派生类 FileNotFoundError
...
- 用关键字 `except` 可以捕捉异常。
  ...
  - 运行逻辑：
    - 首先执行 try 语句块，如果产生了异常，则终止执行 try 语句块，然后，
      - 如果当前异常类型，属于某个 except 指定的异常类或其派生类，则执行该 except 语句块。
...
- 例：
  ...
  ```
  - Exception 是大部分类型的异常的基类。因此，如果在 except 语句块中指定 Exception 类，则可以匹配大部分类型的异常。
