...
- Python 中，没有 char 数据类型，只有 str 数据类型，用于存储一个字符串。
  - str 的用法像 tuple 数据类型，但包含的每个元素都是一个字符，不能是 int 等数据类型。
    ```py
    >>> a = 'hello'
    >>> a[0]    # 可以索引
    'h'
...
- 如何将 list 对象转换成 str 对象？
  - 直接用 str() 转换，会保留 list 的逗号、中括号。
    ```py
    >>> str([1, 2, 3])
    '[1, 2, 3]'
    ```
  - 如果 list 中的元素都是 str 类型，则可以用 join() 合并成一个字符串。
    ```py
    >>> ' '.join(['Hello', 'World'])
    'Hello World'
    ```
  - 如果 list 中的元素不一定是 str 类型，则转换成 str 类型之后，才能用 join() 合并。
    ```py
    >>> a = [1, 2, 3]
    >>> ' '.join(a)
