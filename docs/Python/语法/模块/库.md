...
：Python 的第三方库。是一个 Python 软件包管理工具，相当于改进版的 easy_install ，可以下载、安装、卸载 Python 软件包。
...
pipenv
      ...
      sync                # 安装 Pipfile.lock 文件中指定的所有包
...
：Python 的第三方库。相当于改进版的 pipenv 。
- [官方文档](https://docs.astral.sh/uv/)

- uv 的速度，比 pip 快了十倍。主要因为：
  - uv 采用 Rust 语言开发，分析依赖关系的速度更快。
  - 采用全局缓存。每个 Python 项目下载包时，会将文件存储在当前主机的 cache 目录下，然后在 Python 项目的 .venv 目录下，创建一个指向 cache 的硬链接文件。
...
- 命令：
  ...
  uv
    init <name>   # 新建一个项目
                  # 这会根据项目名，创建一个子目录，生成 .gitignore、pyproject.toml 等文件
    venv <dir>    # 创建一个虚拟环境，存储在指定目录
      --python 3.12 # 指定 Python 解释器的版本。如果本机未安装，则会自动从互联网安装
    python
      list          # 显示本机已安装的所有 Python 解释器
      find 3.9      # 显示某个版本的文件路径
                    # 执行 uv python find ，会显示当前虚拟环境的 Python 路径，默认为 .venv/bin/python3
      install 3.9   # 安装指定版本
      uninstall 3.9 # 卸载指定版本
    run           # 使用虚拟环境，运行一个目标
      python
      run.py
    ...
    remove <name>...  # 删除包
    lock          # 生成 uv.lock 文件
    sync          # 安装 uv.lock 文件中指定的所有包。如果当前项目不存在虚拟环境，则会自动创建
      --no-cache  # 不使用全局缓存
    cache
      clean       # 删除全局缓存中的所有包
      prune       # 删除未使用的包
      dir         # 查看缓存目录，默认位于 /root/.cache/uv
      size
  ...
  ```
  - 2021 年，Python 官方通过了 PEP 621 标准，建议大家用 pyproject.toml 文件描述 Python 项目，取代以前的 setup.py 文件。
    - 配置语法： <https://packaging.python.org/en/latest/guides/writing-pyproject-toml/>
    - uv、poetry 等包管理工具，强制用户在 pyproject.toml 文件中，声明希望安装哪些 dependencies ，取代 requirements.txt 文件。
      - 还会自动生成一个 lock 文件，声明实际安装了哪些 dependencies 。
