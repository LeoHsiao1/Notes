...
- Python 语言中，for 语句的功能不同，是遍历（或者说迭代）一个对象中的所有元素，每获取一个元素，就执行一次循环。
  - 目标对象必须属于可迭代对象。
  - 一般格式如下：
    ```py
    for var in iterable:
  ...
  - 例：
    ...
    for _ in range(4):  # 如果不想遍历对象，只想执行指定次数的循环，则可以用 range() 函数创建一个指定长度的迭代器
        print(1)
    ```
  - 每次迭代得到的这个元素，如果也属于可迭代对象，则可以 unpack 拆开成 n 个值，同时赋值给 n 个变量。例如：
    ```py
    >>> for x,y in ['ab', 'cd']:
    ...     print(x,y)
    ...
...
- Python 的 for、while 循环语句块中，
  - 可以用关键字 break ，跳出当前的循环语句块。
  - 可以用关键字 continue ，跳到下一次循环，但依然位于当前的循环语句块。
  - 可以附加 else 语句块。如果循环语句块正常结束，则会执行 else 语句块。如果因为 break、异常等原因提前结束循环，则不会执行 else 语句块。例如：
    ```py
    >>> for i in range(3):
    ...     print(i)
    ... else:
