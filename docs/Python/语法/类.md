...
- C++ 支持方法重写（override）、方法重载（overload），从而实现多态性。而 Python 只支持 override 。
  - Python 中，如果在一个类中，定义了名称相同的多个方法，则只会保留最后一个定义的方法。
  - Python 中，如果子类定义了一个方法，与父类的某个方法同名，则会覆盖父类的这个方法，不再继承它，但依然可以通过 `super().方法名()` 来调用它。
...
- C++ 中可以将一个类，声明为抽象类（abstract class，简称为 abc）。
  - 一个抽象类，不能被直接实例化，只能专门用作基类，被其它类继承。
  - 一个抽象类中，可以包含多个抽象方法，和非抽象方法。
  - 当前类继承一个抽象类时，必须 override 其所有抽象方法，除非当前类也是抽象类。

- Python 原生语法中，不存在抽象类。但可以调用标准库 abc 来实现这一需求。
  - 例：创建一个抽象类
    ```py
    >>> from abc import ABCMeta, abstractmethod
    >>> class Test(metaclass=ABCMeta):  # 设置元类
    ...     @abstractmethod             # 声明抽象方法
    ...     def method1(self):
    ...         pass
    ...    @staticmethod
    ...    @abstractmethod              # 它可以与其它装饰器组合使用，但必须位于最下方
    ...    def method2():
    ...        pass
    ...
    >>> Test()
    ...
    ```
  - 例如，标准库 collections 提供了一些抽象类，比如 Iterator、Iterable ，常用于类型检查。
