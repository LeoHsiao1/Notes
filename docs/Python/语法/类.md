...
- Python 属于动态语言，允许给一个已经定义的类，添加成员。
  - 例：添加实例变量
    ```py
    >>> class Test:
    ...     pass
...
- 因此，给一个类或实例，的属性进行赋值时，即使该属性不存在，也可以赋值成功。这可能引发 bug ，建议通过类的内置属性 `__slots__` ，声明只允许存在哪些名称的成员。
  ```py
  >>> class Test:
  ...
  ```
  - 定义一个类时，如果未声明 `__slots__` ，则不会继承父类的 `__slots__` ，因此允许添加任何成员。
...
- 如何定义实例方法？
  - 在类中，像函数一样定义，但第一个形参是特殊的，通常命名为 `self` 。
    ...
    - `self` 有什么用处？通过它，可以访问当前实例的实例变量、实例方法。
...
- `@property` 是 Python 的一个内置装饰器，用于将类的某个方法，转换成只读属性。
  - 优点：当用户读取该属性时，会调用该方法，从而能执行一些业务代码，例如记录该属性被读取了多少次。
  - 例：
    ```py
    >>> class Test:
    ...     def __init__(self):
...
- 其它内置方法：
  - 定义了 `__enter__()` 和 `__exit__()` 方法的对象，支持用关键字 with 进行上下文管理。
  - `__sizeof__()`
    - ：返回对象占用的内存大小，单位为 bytes 。
    - 例：
      ```py
      a = 1
      a.__sizeof__()
...
- `__call__()`
  ...
  - 例：
    ...
    ```
- `__getattr__()`
  - ：当用户访问对象的一个成员时，如果该成员未定义，则自动调用 `__getattr__()` 来查找。如果依然找不到，则抛出 AttributeError 异常。
  - 例：
    ```py
    >>> class Test:
