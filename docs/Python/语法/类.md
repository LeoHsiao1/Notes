...
- 因此，给一个类或实例，的某个属性进行赋值时，即使该属性不存在，也可以赋值成功，这可能引发 bug 。
  - 建议声明类的内置变量 `__slots__`（取值为一个 tuple ），表示只允许添加哪些名称的属性。
    ```py
    >>> class Test:
    ...     __slots__ = ('name',)
    ...
...
- `@property` 是 Python 的一个内置装饰器，用于将类的某个方法，转换成变量。
  ...
  - 例：
    ...
    ...     @property               # 这个装饰器的作用，是将 sum 方法转换成一个只读变量
    ...     def sum(self):
    ...         return self._sum
    ...     @sum.setter             # @变量名.setter 这个装饰器的作用，是让变量 sum 允许被赋值
