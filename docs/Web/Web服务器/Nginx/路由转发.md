...
- 例：
  ...
  server {
      listen 80;
      location / {
          proxy_pass  http://127.0.0.1:81;                    # upstream 地址可以是 IP，也可以是upstream定义的上游名称。
...
：对 upstream 发出的响应头中的 Location、Refresh 字段进行字符串替换。nginx 的 proxy_redirect 指令用于设置反向代理服务器在将响应返回给客户端时，是否修改响应报文中的 Location 和 Refresh 头域的值。

默认情况下，反向代理服务器会将原始的 Location 和 Refresh 头域的值直接返回给客户端。但是，如果这些值中包含了反向代理服务器的地址，客户端浏览器会直接向反向代理服务器发起请求，而不是向原始服务器发起请求。这就会导致浏览器无法正确地访问原始服务器。

为了解决这个问题，可以使用 proxy_redirect 指令来修改响应报文中的 Location 和 Refresh 头域的值，将其中包含反向代理服务器地址的部分替换为原始服务器地址。这样，客户端浏览器就会向原始服务器发起请求，而不是向反向代理服务器发起请求。
- 可用范围：http、server、location
- 例：
  ```sh
  location /www/ {
