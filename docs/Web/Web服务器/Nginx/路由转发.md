...
- 例：
  ...
  - server 的可用参数：
    ...
    max_fails=1       # 默认为 1 ，取值为 0 则不限制
    fail_timeout=10s  # 默认为 10 秒。如果在一个 fail_timeout 周期内，与该 server 通信失败 max_fails 次，则在该周期内认为它不可用
...
如果 upstream 中存在多个 server ，则 nginx 会将 HTTP 请求转发给哪个 server ？
- 轮询
  - 按 HTTP 请求依次分配给各个 server ，实现简单的平均分配。
  - 配置示例：
    ```sh
    upstream backend {
        server 127.0.0.1:81;
        server 127.0.0.1:82;
        server test.backend.com resolve;
          # Nginx 在启动时，会对 upstream 中声明的域名进行一次 DNS 解析。如果一个域名解析到多个 IP ，则对这些 IP 进行轮询分配
          # 此时需要配置 resolver ，指定 DNS 服务器。并用 zone 指令创建共享内存，暂存解析结果
          # resolver 8.8.8.8 valid=1s;
          # zone test 64k;
    }
    ```
  - 如果一个 server 不可用，则分配给下一个 server 处理。如果所有 server 都不可用，则将最后一个 server 的报错，返回给客户端。

- 加权轮询（weighted round-robin）
  - 这是 nginx 的默认分配策略。
  - 权重越大的 server ，被分配 HTTP 请求的概率越大。这样适合处理几台 server 性能不均的情况。
  - 配置示例：
    ```sh
    upstream backend {
...
- 最短响应时间
  - 响应时间越短的 server ，被分配 HTTP 请求的概率越大。
  - 配置示例：
    ```sh
    upstream backend {
        fair;
...
- 最少连接数
  - 找出 TCP 活跃连接数最少的一个 server ，将 HTTP 请求分配给该 server 。
    - 假设一个 server 存在 n 个 TCP 活跃连接数，则说明正在并发传输 n 个 HTTP 报文。
  - 配置示例：
    ```sh
    upstream backend {
        least_conn;
        server 127.0.0.1:81;
        server 127.0.0.1:82;
          # 可以配置 weight ，同时启用加权轮询算法
          # 如果两个 server 的 TCP 活跃连接数相等，则比较 weight 大小，决定分配给哪个 server
    }
    ```

- 最短响应时间
  - 找出平均响应时间最短，并且 TCP 活跃连接数最少的一个 server ，将 HTTP 请求分配给该 server 。
  - 配置示例：
    ```sh
    upstream backend {
        least_time;
...
- ip 哈希
  - 对 ip 的前 24 位计算 hash 值，将 hash 值相同的 HTTP 请求分配给同一个 server 。这样适合保持 session 、利用缓存。
  - 配置示例：
    ```sh
    upstream backend {
        ip_hash;
  ...
  stream {
      ...
      server {
          ...
          proxy_connect_timeout 5s;
            # 与 upstream 建立连接的超时时间，默认为 1m
          proxy_timeout 1m;
            # 如果超过一定时间未读写数据，则断开连接。默认为 10m
          # proxy_next_upstream on;
            # 不能与 upstream server 建立连接时，是否自动尝试连接下一个 upstream server
          # proxy_bind $remote_addr transparent;
            # 将转发的数据包的源地址 ip:port 改为真实的源地址，实现透明代理，此时需要以 user root 运行 Nginx ，且 upstream 会返回响应给该地址。默认采用当前 Nginx 的地址
      }
  }
  ```
