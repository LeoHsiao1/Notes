...
- 例：
  ...
  ```
  - alias 指令会覆盖 root 指令的效果。
    - 上例中，如果没有 alias 指令，则请求 /www/1.jpg 时，会尝试获取 /usr/share/nginx/html/1.jpg 文件，即最终文件路径为 `$root/$uri` 。
    - 有了 alias 指令，则请求 /www/1.jpg 时，会尝试获取 /static/img/1.jpg 文件，即最终文件路径为 `uri.replace(root, alias)` 。
...
- 例：
  ...
  server {
      ...
      location / {
          ...
          # proxy_request_buffering on;     # 接收客户端的请求时，缓冲之后再转发给 upstream
...
当 Nginx 执行 proxy_pass 接收 upstream 的响应报文时，默认启用了缓冲，但没有启用缓存。
- 缓冲（buffer）
...
- 缓存（cache）
  - ：暂存几分钟甚至几天，当客户端再次请求同一个响应报文时就拿缓存文件回复，不必请求 upstream 。
  - 优点：
