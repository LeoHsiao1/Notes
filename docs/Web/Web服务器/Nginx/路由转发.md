...
- 例：
  ...
  server {
      ...
      location / {
          ...
          # proxy_ssl_server_name off;      # 与 upstream 进行 SSL 握手时，是否发送 SNI 。如果 upstream 有多个 SSL 证书，则需要启用 SNI ，否则会选用默认的 SSL 证书
      }
  }
...
- 缓冲（buffer）
  ...
  - 缺点：
    - 增加了客户端等待响应的时间。
  - 如果不启用缓冲，则 Nginx 每接收 proxy_buffer_size 大小的响应数据就同步发送给客户端。这样通信延迟更低，但是效率低。
