...
- 例：
  ...
  server {
      listen 80;
      location / {
          proxy_pass  http://127.0.0.1:81;                    # 转发地址可以是 IP
          # proxy_pass http://host1.test.com;                 # 转发地址可以是域名。Nginx 只会在 reload 时执行一次 DNS 查询，解析到 IP 并缓存使用，如果解析失败则报错
          # proxy_pass http://unix:/tmp/backend.socket:/uri/; # 转发地址可以是 Unix 套接字
...
- 例：
  ```sh
  upstream backend {              # 定义一个 upstream ，名为 backend
  ...
  server {
      listen 80;
      location / {
          proxy_pass  http://backend;  # 将请求转发给上游服务器
      }
  }
  ```
