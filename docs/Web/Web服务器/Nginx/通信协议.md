...
：用于启用 Nginx 到 upstream 的 TCP 长连接。
- 可用范围：upstream
- 例：
  ```sh
...
- Nginx 默认启用长连接，但反向代理 upstream 时默认禁用长连接。
  - 此时，upstream 发出响应报文之后，会主动关闭连接，让 Socket 变成 TIME_WAIT 状态。
...
- 例：启用 Nginx 到 upstream 的长连接
  ...
  ```
  - 此时，由 Nginx 主动关闭长连接，会让 Nginx 上的 Socket 变成 TIME_WAIT 状态。增加 keepalive_requests 可以避免频繁关闭长连接。
