...
- 可用范围：upstream
- Nginx 担任服务器时，默认启用 TCP 长连接。
  - 因此客户端向 nginx 建立的 TCP 长连接会一直保持，直到客户端主动关闭连接，或者达到 keepalive_timeout 等限制。
- 用 proxy_pass 直接反向代理 ip:port 时，Nginx 担任客户端，默认禁用 TCP 长连接。
  - 需要按以下格式配置 nginx ，才能启用 nginx 到 upstream 的长连接：
    ```sh
    upstream backend {
        server 127.0.0.1:80;
        keepalive 8;  # 启用 TCP 长连接，并限制每个 worker 最多存在的空闲连接数，但并不限制长连接总数
        # keepalive_requests 1000;  # 限制每个 TCP 长连接可以发送的请求数。如果超过，则关闭该连接
        # keepalive_time 1h;        # 限制每个 TCP 长连接的存在时间。如果超过，则关闭该连接，除非当前还在传输 HTTP 报文
        # keepalive_timeout 60s;    # 限制每个 TCP 长连接的空闲时间。如果超过，则关闭该连接
    }
    server {
        listen 80;
        location / {
            ...
            proxy_http_version 1.1;
              # HTTP/1.1 之前的版本，不支持 keepalive
            proxy_set_header Connection '';
              # Connection 字段的默认值为 close ，使得 upstream server 在发送 HTTP 响应之后立即关闭 TCP 连接
        }
    }
    ```
  - 这样，nginx 每次收到 HTTP 请求，会从 upstream keepalive 连接池中，选择一个空闲的 TCP 连接（即当前没有传输 HTTP 报文），传输该 HTTP 请求。
    - 如果连接池中，不存在空闲的 TCP 连接，则自动新建一个 TCP 连接。
    - 传输完 HTTP 请求、响应报文之后，该 TCP 连接就变为空闲。
  - nginx 会为这个 upstream 创建一个双向队列，记录各个空闲的 TCP 连接。
    - 每当一个 TCP 连接变为空闲，就将它插入队头。
    - 每当需要使用一个 TCP 连接，就从队头取出一个连接。这样的连接是最近使用过的，TCP 通信的拥塞控制效果最好。
      - 如果 upstream 中存在多个 server ip ，则需要根据负载均衡算法，选择一个 server ip ，然后从队头遍历，找到匹配该 server ip 的一个空闲连接。
    - 如果空闲的 TCP 连接数，超过 keepalive 限制数量，则关闭队尾的空闲连接，也就是最久未被使用的连接。
...
- 配置示例：
  ...
  ```
  - 大部分浏览器只支持在 HTTPS 端口启用 HTTP/2 协议，因此需要 Nginx 在监听时，同时采用 ssl 和 http2 。

- nginx v1.25 修改了配置语法，要求用单独一行配置，启用 http2 。如下：
  ```sh
  server {
      listen 443 ssl;
      http2 on;
