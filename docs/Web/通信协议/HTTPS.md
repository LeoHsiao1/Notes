...
- client 与 server 进行 SSL 握手时，会下载其 SSL 证书文件，用于验证 server 的身份真伪：
  - 检查该证书的颁发者是谁。如果不是 client 信任的 CA 机构，则该证书无效，SSL 握手失败。
  - 检查该证书颁发给哪些域名使用。如果不包含当前 server 的域名，则该证书无效，SSL 握手失败。
  - 提取证书中的公钥，用它加密一个消息发送给 server 。如果 server 没有对应的私钥，就不能解密该消息，导致 SSL 握手失败。
...
- 如何使用 HTTPS 协议？关键是准备 SSL 证书，这里分情况讨论：
  ...
  - 对于客户端：
    - Chrome 等浏览器软件，通常预先安装了一些主流 CA 机构的根证书，用于验证 HTTPS 服务器的 SSL 证书。因此既能访问 HTTP 服务器，又能访问 HTTPS 服务器。
      - 如果 SSL 证书是从冷门 CA 机构创建的，则需要用户手动安装其根证书到浏览器中。
    - JVM 等程序，通常会认可操作系统已下载的根证书。例如：
      - CentOS 系统默认将 SSL 证书文件存储在 `/etc/pki/tls/certs/` 目录下（包括一些主流 CA 机构的根证书），将私钥文件存储在 `/etc/pki/tls/private/` 目录下。
...
- 相关历史：
  - 1970 年代，英国情报机构开始研发一种非对称加密的系统架构，称为公开密钥基础设施（Public Key Infrastructure，PKI）。
  - 1990 年代，SSL 协议发明之后，采用 PKI 架构管理 SSL 证书，沿用至今。
  - 2002 年，IETF 制定了 PKI 的 X.509 标准。

- 公开密钥基础设施（Public Key Infrastructure，PKI）
  - ：一种系统架构，包含多个角色，定义了如何创建、使用、管理数字证书及其公钥、私钥。

- 证书授权中心（Certificate Authority ，CA）
  - ：PKI 系统中，CA 机构负责颁发数字证书、存储公钥。
  - 多个 CA 机构可以建立树形关系：
    - 上级 CA 可以向下级 CA 颁发证书，证明后者的身份。如果用户信任上级 CA ，则会连带信任其下级 CA 。
    - 最上级 CA 使用一个自签名的证书，称为根证书，作为信任的源头。如果用户信任根证书，则会信任其下级所有 CA 。
  - 互联网上有一些公开可信的 CA 机构。Chrome 等浏览器软件通常预先安装了这些 CA 的根证书，因此信任这些 CA 及下级 CA 创建的所有数字证书。
...
- SSL 数字证书（digital certificate）包含以下信息：
  - SSL 协议版本
  - SSL 公钥
  - 证书的颁发者，即 CA 机构的名称。
...
- 根据 PKI 的 X.509 标准，数字证书保存为一个文件，有两种文件格式：
  - PEM
    - ：文本文件，采用 Base64 编码。
    - 文件扩展名为 .pem、.crt 等。
    - 例如 baidu.com 的数字证书：
